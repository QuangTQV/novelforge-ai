      ...
      <div className={`relative flex items-center justify-center bg-gradient-to-br from-muted/30 to-muted/60 ${imageWrapperClass}`}>
        {hasDoneImage ? (
          ...
          <div className="flex flex-col items-center gap-1.5 text-muted-foreground">
            ...
            <span className="text-[10px]">{translateUi("生成中")}</span>
          </div>
        ) : status === "error" ? (
          <div className="flex flex-col items-center gap-1 px-2 text-center text-rose-600 dark:text-rose-400" title={errorMessage}>
            ...
            <span className="text-[10px]">{translateUi("生成失败，可重试")}</span>
          </div>
        ) : emptyHint ? (
          <>{emptyHint}</>
        ) : (
          <div className="flex flex-col items-center gap-1 text-muted-foreground/50">
            ...
            <span className="text-[10px]">{translateUi("待生成")}</span>
          </div>
        )}
      </div>
          ...
          <div className="flex items-center gap-1.5 pt-0.5">
            {onGenerate && (
              ...
              >
                ...
                {generateLabel ?? (hasDoneImage ? translateUi("重新生成") : translateUi("AI 生图"))}
              </button>
            )}
            {onUpload && (
              <button
                type="button"
                title={translateUi("上传图片替代 AI 生成")}
                disabled={isGenerating}
