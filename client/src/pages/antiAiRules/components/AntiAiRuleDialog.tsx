...
export default function AntiAiRuleDialog(props: AntiAiRuleDialogProps) {
  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <AppDialogContent
        className="max-w-4xl"
        title={props.editingRule ? translateUi("编辑反 AI 规则") : translateUi("新建反 AI 规则")}
        description={translateUi("规则可以进入全局默认，也可以只作为写法资产的可选约束。")}
        footer={(
          <>
            <Button type="button" variant="outline" onClick={() => props.onOpenChange(false)}>{translateUi("取消")}</Button>
            <Button type="submit" form="anti-ai-rule-form" disabled={props.isSaving}>
              {props.isSaving ? translateUi("保存中...") : translateUi("保存规则")}
            </Button>
          </>
        )}
      >
        <form id="anti-ai-rule-form" className="space-y-4" onSubmit={props.onSubmit}>
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-1">
                ...
                <div className="text-sm leading-6 text-muted-foreground">
                  {props.editingRule
                    ? translateUi("描述要调整的方向，AI 会基于表单内容优化规则。")
                    : translateUi("描述想压制或鼓励的表达，AI 会生成一条可编辑规则草稿。")}
                </div>
              </div>
              <Button
                type="button"
              ...
              >
                <Sparkles className="h-4 w-4" />
                {props.isAiDrafting ? translateUi("生成中...") : props.editingRule ? translateUi("AI 优化草稿") : translateUi("AI 生成草稿")}
              </Button>
            </div>
            <textarea
              ...
              placeholder={props.editingRule
                ? translateUi("例如：把这条规则改得更适合压制总结腔，但不要误伤正常心理描写。")
                : translateUi("例如：减少正文里空泛总结、解释人物心理、像模型在复盘剧情的表达。")}
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <label className="space-y-1.5 text-sm">
              <span className="font-medium">{translateUi("规则标识")}</span>
              <Input
                value={props.form.key}
                placeholder={translateUi("例如 direct_psychology_explain")}
                onChange={(event) => props.onFormChange({ key: event.target.value })}
              />
            </label>
            <label className="space-y-1.5 text-sm">
              <span className="font-medium">{translateUi("规则名称")}</span>
              <Input
                value={props.form.name}
                placeholder={translateUi("例如 避免直白心理解释")}
                onChange={(event) => props.onFormChange({ name: event.target.value })}
              />
            </label>
            <label className="space-y-1.5 text-sm">
              <span className="font-medium">{translateUi("规则类型")}</span>
              <Select value={props.form.type} onValueChange={(value) => props.onFormChange({ type: value as AntiAiRule["type"] })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="forbidden">{translateUi("禁用")}</SelectItem>
                  <SelectItem value="risk">{translateUi("风险")}</SelectItem>
                  <SelectItem value="encourage">{translateUi("鼓励")}</SelectItem>
                </SelectContent>
              </Select>
            </label>
            <label className="space-y-1.5 text-sm">
              <span className="font-medium">{translateUi("严重度")}</span>
              <Select value={props.form.severity} onValueChange={(value) => props.onFormChange({ severity: value as AntiAiRule["severity"] })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="low">{translateUi("低")}</SelectItem>
                  <SelectItem value="medium">{translateUi("中")}</SelectItem>
                  <SelectItem value="high">{translateUi("高")}</SelectItem>
                </SelectContent>
              </Select>
            </label>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <label className="space-y-1.5 text-sm">
              <span className="font-medium">{translateUi("生成指令")}</span>
              <textarea
                className="min-h-[120px] w-full rounded-md border bg-background p-3 text-sm"
                value={props.form.promptInstruction}
                placeholder={translateUi("写进正文生成约束的具体表达要求。")}
                onChange={(event) => props.onFormChange({ promptInstruction: event.target.value })}
              />
            </label>
            <label className="space-y-1.5 text-sm">
              <span className="font-medium">{translateUi("修正建议")}</span>
              <textarea
                className="min-h-[120px] w-full rounded-md border bg-background p-3 text-sm"
                value={props.form.rewriteSuggestion}
                placeholder={translateUi("检测命中后给用户或改写链路的调整建议。")}
