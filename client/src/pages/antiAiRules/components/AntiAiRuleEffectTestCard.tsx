    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ...
          {translateUi("粘贴一段正文，检查规则会怎样判断 AI 味，并生成一版只用于预览的修订稿。")}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-2 text-sm sm:grid-cols-2">
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs text-muted-foreground">{translateUi("测试规则")}</div>
            <div className="mt-1 font-semibold">{totalRuleCount}</div>
          </div>
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs text-muted-foreground">{translateUi("临时加入")}</div>
        ...
        {props.previewRules.length > 0 ? (
          <div className="space-y-2 rounded-md border bg-muted/20 p-3">
            <div className="flex items-center justify-between gap-2">
              <div className="text-sm font-medium text-foreground">{translateUi("临时测试规则")}</div>
            ...
            <div className="flex flex-wrap gap-2">
              {props.previewRules.map((rule) => (
                <button
                  ...
                  title={translateUi("移出测试")}
                >
                  {translateUi(rule.name)}
                  <X className="h-3 w-3" />
                </button>
              ))}
            </div>
          ...
          <div className="rounded-md border border-dashed p-3 text-sm leading-6 text-muted-foreground">
            ...
            {translateUi("从左侧规则列表加入测试，可以在不改变规则状态的情况下比较效果。")}
          </div>
        )}
        ...
        {props.report ? (
          <div className="space-y-3 rounded-md border p-4">
            <div className="space-y-1">
              <div className="font-medium text-foreground">{translateUi("风险分：")}{props.report.riskScore}</div>
              <div className="text-sm leading-6 text-muted-foreground">{props.report.summary}</div>
              <div className="text-xs text-muted-foreground">{translateUi("命中规则：")}{props.report.appliedRuleIds.length}</div>
            </div>
              ...
              <div className="space-y-2">
                {props.report.violations.map((item, index) => (
                  <div key={`${item.ruleId}-${index}`} className="rounded-md border bg-muted/20 p-3">
                    ...
                    <div className="mt-2 text-xs leading-5 text-muted-foreground">{translateUi("建议：")}{item.suggestion}</div>
                  </div>
                ))}
              </div>
            ) : (
        ...
        {props.rewritePreview ? (
          <div className="space-y-2">
            <div className="text-sm font-medium text-foreground">{translateUi("修订稿预览")}</div>
