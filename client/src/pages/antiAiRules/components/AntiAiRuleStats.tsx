...
export default function AntiAiRuleStats(props: AntiAiRuleStatsProps) {
  return (
    <div className="grid gap-3 md:grid-cols-4">
      <StatTile label={translateUi("规则总数")} value={props.total} hint={translateUi("规则库中可查看和编辑的全部规则。")} />
      <StatTile label={translateUi("启用规则")} value={props.enabled} hint={translateUi("会参与全局或写法绑定解析的规则。")} />
      <StatTile label={translateUi("全局默认")} value={props.global} hint={translateUi("不绑定写法时也会进入正文生成。")} />
      <StatTile label={translateUi("自动改写")} value={props.autoRewrite} hint={translateUi("检测命中后可进入改写建议链路。")} />
    </div>
  );
}
