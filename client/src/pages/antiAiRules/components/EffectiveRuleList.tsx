          ...
          {props.rules.map((item) => (
            <div key={`${item.source}-${item.rule.id}`} className="rounded-md border bg-background p-3">
              <div className="flex flex-wrap items-center gap-2">
                <div className="font-medium text-foreground">{translateUi(item.rule.name)}</div>
                <Badge variant={item.source === "global_baseline" ? "default" : "secondary"}>
                  {item.source === "global_baseline" ? translateUi("全局默认") : translateUi("写法规则")}
                </Badge>
                <Badge variant="outline">{typeLabels[item.rule.type]} / {severityLabels[item.rule.severity]}</Badge>
              </div>
              <div className="mt-2 text-xs leading-5 text-muted-foreground">
                {item.sourceLabel}{item.weight !== 1 ? translateUi("，强度 {{value0}}", { value0: item.weight }) : ""}
              </div>
              {item.rule.promptInstruction ? (
                <div className="mt-2 text-sm leading-6 text-muted-foreground">{translateUi(item.rule.promptInstruction)}</div>
              ) : null}
            </div>
          ))}
        </div>
