...
export default function BookAnalysisBudgetAdjustDialog(props: BookAnalysisBudgetAdjustDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <AppDialogContent
        title={mode === "resume" ? translateUi("扩容预算并续跑") : translateUi("调整拆书预算")}
        description={mode === "resume"
          ? translateUi("为这次拆书设置新的预算上限，并继续处理未完成的小节。")
          : translateUi("修改预算上限后，累计用量保留，后续小节按新的上限检查。")}
        className="max-w-xl"
        footer={
          <div className="flex w-full justify-end gap-2">
            ...
            <Button type="button" onClick={handleSubmit} disabled={!canSubmit}>
              {pending ? translateUi("提交中...") : mode === "resume" ? translateUi("扩容并续跑") : translateUi("保存调整")}
            </Button>
          </div>
        }
      >
        <div className="space-y-4">
          {mode === "resume" ? (
            <div className="rounded-md border border-warning/30 bg-warning/5 p-3 text-sm leading-6 text-foreground">
              ...
              {translateUi("本次会重做")} {retrySectionCount}  {translateUi("节，成功的")} {succeededSectionCount}  {translateUi("节保留")}
              {frozenSectionCount > 0 ? translateUi("，另有 {{value0}} 节不纳入本次续跑", { value0: frozenSectionCount }) : ""}{translateUi("。")}
            </div>
          ) : null}

          <div className="grid gap-2 rounded-md border bg-muted/20 p-3 text-sm sm:grid-cols-3">
            <div>
              <div className="text-xs text-muted-foreground">{translateUi("累计用量")}</div>
              <div className="mt-1 font-mono tabular-nums">{formatTokenCount(usedTokens)}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">{translateUi("预算上限")}</div>
              <div className="mt-1 font-mono tabular-nums">
                {currentBudget ? formatTokenCount(currentBudget) : translateUi("不限")}
              </div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">{translateUi("调整后剩余")}</div>
              <div className="mt-1 font-mono tabular-nums">
                {parsedBudget === null
                  ? translateUi("不限")
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-3">
              ...
              {mode === "resume" ? (
                ...
                >
                  ...
                  {translateUi("使用建议值")} {formatTokenCount(recommendedResumeBudget)}
                </button>
              ) : null}
            </div>
            <div className="flex items-center gap-2">
              <Input
                ...
                placeholder={allowUnlimited ? translateUi("留空表示不限") : String(recommendedResumeBudget)}
            ...
            {!hasValidBudget ? (
              <div className="text-xs text-destructive">
                ...
                {translateUi("请输入")} {formatTokenCount(MIN_BUDGET_TOKENS)}  {translateUi("到")} {formatTokenCount(MAX_BUDGET_TOKENS)}  {translateUi("之间的整数。")}
              </div>
            ) : null}
            {mode === "adjust" && analysis.status === "running" ? (
              <div className="text-xs leading-5 text-muted-foreground">

                {translateUi("调低预算不会立即终止当前小节，会在下个小节边界按新上限检查。")}
              </div>
            ) : null}
            {budgetIsFinite && remainingTokens !== null && remainingTokens < 0 ? (
              <div className="text-xs leading-5 text-warning">

                {translateUi("新预算低于累计用量，继续生成时会触发预算停止。")}
              </div>
            ) : null}
          </div>
        </div>
