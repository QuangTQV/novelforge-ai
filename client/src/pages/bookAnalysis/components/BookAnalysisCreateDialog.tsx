...
export default function BookAnalysisCreateDialog(props: BookAnalysisCreateDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <AppDialogContent
        title={translateUi("新建拆书分析")}
        description={translateUi("选择文档与拆书范围，提交后会在右侧分析列表中出现新任务。")}
        className="max-w-4xl"
        footer={
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
          <div className="space-y-4">
            ...
            {isDiagnosisMode ? (
              <div className="space-y-2">
                <div className="text-sm font-medium">{translateUi("要诊断的小说")}</div>
                <SelectControl
                ...
                <div className="rounded-md border bg-muted/20 p-3 text-xs leading-5 text-muted-foreground">
                  ...
                  {translateUi("系统会导出这本小说的当前章节正文，作为新的知识文档创建诊断拆书。")}
                </div>
              </div>
            ) : (
              <>
                ...
                <div className="space-y-2">
                  <div className="text-sm font-medium">{translateUi("知识文档")}</div>
                  <SelectControl
                    className="h-10 w-full rounded-md border bg-background px-3 text-sm"
                    value={selectedDocumentId}
                    onChange={(event) => onSelectDocument(event.target.value)}
                  >
                    <option value="">{translateUi("选择文档")}</option>
                ...
                <div className="space-y-2">
                  <div className="text-sm font-medium">{translateUi("文档版本")}</div>
                  <SelectControl
                    className="h-10 w-full rounded-md border bg-background px-3 text-sm"
                  ...
                  >
                    <option value="">{translateUi("使用当前激活版本")}</option>
                    {versionOptions.map((version) => (
                      <option key={version.id} value={version.id}>
                        v{version.versionNumber} {version.isActive ? translateUi("（当前）") : ""}
                      </option>
                    ))}
                  </SelectControl>
                </div>
            ...
            <div className="space-y-2">
              <div className="text-sm font-medium">{translateUi("模型")}</div>
              <LLMSelector
                value={llmConfig}
                onChange={(next) =>
                  onLlmConfigChange({
              ...
              <div className="grid gap-2 rounded-md border bg-muted/20 p-3 sm:grid-cols-[minmax(0,1fr)_180px] sm:items-center">
                <div>
                  <div className="text-sm font-medium">{translateUi("预算上限")}</div>
                  <div className="mt-1 text-xs leading-5 text-muted-foreground">

                    {translateUi("留空使用服务端默认值。累计用量达到上限后停止任务，已完成的小节会保留。")}
                  </div>
                </div>
            ...
            <div className="space-y-2">
              <div className="text-sm font-medium">{translateUi("分析维度")}</div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="text-xs text-muted-foreground">{preset.sectionKeys.length}  {translateUi("项")}</div>
                      </div>
                      <div className="mt-1 text-xs leading-5 text-muted-foreground">{preset.summary}</div>
                      <div className="mt-2 text-xs leading-5 text-muted-foreground">

                        {translateUi("包含：")}{getPresetSectionTitles(preset.sectionKeys)}
                      </div>
                    </button>
                  );
                })}
            ...
            <div className="space-y-2">
              <div className="text-sm font-medium">{translateUi("本次拆书重点")}</div>
              <textarea
                ...
                placeholder={isDiagnosisMode
                  ? translateUi("例如：重点检查前三章留存、主角动机清晰度或伏笔回收风险。")
                  : translateUi("例如：重点观察群像戏轮转、主角语言风格或付费爽点设计。")}
              />
            </div>
          </div>
          ...
          <aside className="space-y-3">
            <div className="rounded-md border border-warning/30 bg-warning/5 p-3 text-xs leading-5 text-foreground">
              {isDiagnosisMode
                ? translateUi("诊断会根据小说正文长度消耗模型 token。章节越多，分析时间和 token 用量通常越高；建议先选择适合本次检查的拆书范围。")
                : translateUi("拆书会根据书籍内容长度消耗模型 token。书籍越长，分析时间和 token 用量通常越高；建议先确认文档范围，再开始分析。")}
            </div>
            ...
            {!isDiagnosisMode && sourceDocument ? (
              <div className="rounded-md border bg-muted/20 p-3 text-xs text-muted-foreground">
                ...
                {translateUi("版本数：")}{sourceDocument.versions.length}  {translateUi("| 已有拆书：")}{sourceDocument.bookAnalysisCount}
              </div>
            ) : null}
          </aside>
        </div>
