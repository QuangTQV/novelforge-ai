    ...
    <Card className="overflow-hidden rounded-2xl border-border/50 bg-card/80 shadow-[0_14px_40px_rgba(15,23,42,0.04)] backdrop-blur-sm">
      <CardHeader className="space-y-4 border-b border-border/40 px-4 pb-4 pt-5">
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="text-lg tracking-tight">{translateUi("分析列表")}</CardTitle>
      ...
      <CardContent className="space-y-3 px-3 pb-4 pt-4">
        <Input className="h-10 rounded-xl border-border/50 bg-muted/20 shadow-none" value={keyword} onChange={(event) => onKeywordChange(event.target.value)} placeholder={translateUi("搜索标题或关键词")} />
        <SelectControl
        ...
        <div className="space-y-1 pt-1">
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    {item.sourceRange ? (
                      <div className="mt-1 truncate text-[11px] text-muted-foreground">{translateUi("范围：")}{item.sourceRange.label ?? translateUi("选定章节")}</div>
                    ) : null}
                  </div>
                  <div className="flex shrink-0 flex-col items-end gap-1">
                    {item.publishedDocumentId && (
                      <Badge variant="secondary" className="border-0 bg-muted px-1.5 text-[10px] font-normal">{translateUi("已发布")}</Badge>
                    )}
          ...
          {!loading && !errorMessage && analyses.length === 0 ? (
            <div className="rounded-md border border-dashed p-4 text-xs text-muted-foreground">
              {keyword.trim() || status
                ? translateUi("没有符合当前筛选的拆书分析，可以调整筛选条件。")
                : translateUi("暂无拆书分析，点击上方「新建拆书」开始。")}
            </div>
          ) : null}
        </div>
      </CardContent>
