...
export function CharacterCard({
  ...
  const handleDeleteAsset = async (asset: ImageAsset) => {
    const confirmed = window.confirm(translateUi("确认删除这张形象图？此操作不可恢复。"));
    if (!confirmed) {
      return;
    }
    try {
    ...
    <article className="overflow-hidden rounded-md border border-border/80 bg-background">
      <div className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="secondary">{character.role || translateUi("未设置定位")}</Badge>
            {character.category && character.category !== character.role ? (
              <Badge variant="outline">{character.category}</Badge>
            ) : null}
          </div>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
            {character.personality || translateUi("还没有性格说明，可以先补充角色面对压力时会如何选择。")}
          </p>
        </div>
        <div className="mobile-full-actions flex shrink-0 flex-wrap items-center gap-2 sm:justify-end">
          ...
          <Button size="sm" variant="outline" onClick={onConversation}>
            <MessageCircle className="mr-1.5 h-3.5 w-3.5" />{translateUi("与角色交谈")}
          </Button>
          ...
          <Button
            ...
            disabled={deleting}
          >
            {deleting ? translateUi("删除中...") : translateUi("删除")}
          </Button>
        </div>
      </div>

      <div className="grid gap-px border-y border-border/70 bg-border/70 sm:grid-cols-2 xl:grid-cols-3">
        <CharacterFact label={translateUi("背景与来处")} value={character.background} />
        <CharacterFact label={translateUi("成长方向")} value={character.development} />
        <CharacterFact label={translateUi("弱点与代价")} value={character.weaknesses} />
        <CharacterFact label={translateUi("外貌与体态")} value={character.appearance} />
        <CharacterFact label={translateUi("习惯与特长")} value={character.interests} />
        <CharacterFact label={translateUi("关键经历")} value={character.keyEvents} />
      </div>

      <div className="space-y-3 px-4 py-4">
        <div>
          <div className="text-sm font-semibold text-foreground">{translateUi("角色形象")}</div>
          <div className="mt-1 text-xs text-muted-foreground">{translateUi("保存主形象后，可在后续视觉生成中保持角色识别度。")}</div>
        </div>
        {assetsLoading ? <div className="text-xs text-muted-foreground">{translateUi("加载中...")}</div> : null}
        {!assetsLoading && assets.length === 0 ? (
          <div className="text-xs text-muted-foreground">{translateUi("暂无图片，点击“生成形象图”创建。")}</div>
        ) : null}
        {assets.length > 0 ? (
            ...
            {assets.map((asset) => (
              <div key={asset.id} className="w-full max-w-[300px] space-y-2 rounded-md border border-border/80 p-2">
                <button
                  ...
                  title={translateUi("点击预览")}
                >
                  <img
                    src={resolveImageAssetUrl(asset.url)}
                    alt={translateUi("{{value0}}-形象图", { value0: character.name })}
                    className="h-full w-full object-cover transition-transform duration-200 hover:scale-[1.02]"
                    loading="lazy"
                  />
                </button>
                <details className="text-[11px] leading-4 text-muted-foreground">
                  <summary className="cursor-pointer select-none">{translateUi("文件详情")}</summary>
                  <div className="mt-1 break-all">{translateUi("本地路径：")}{asset.localPath ?? translateUi("未落地本地文件")}</div>
                </details>
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">{asset.isPrimary ? translateUi("主图") : translateUi("候选图")}</div>
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {deletingAssetId === asset.id ? translateUi("删除中...") : translateUi("删除")}
                    </Button>
                  </div>
                </div>
              </div>
      ...
      >
        <AppDialogContent
          ...
          title={previewAsset ? translateUi("{{value0}} - 图片预览", { value0: character.name }) : translateUi("图片预览")}
          ...
          footer={previewAsset ? (
            <>
              ...
              >
                {deletingAssetId === previewAsset.id ? translateUi("删除中...") : translateUi("删除图片")}
              </Button>
            </>
          ) : null}
            ...
            <>
              <div className="flex max-h-[70vh] items-center justify-center overflow-auto rounded-md bg-muted/30 p-2">
                <img
                  ...
                  alt={translateUi("{{value0}}-预览图", { value0: character.name })}
              ...
              {previewAsset.localPath ? (
                <div className="text-xs text-muted-foreground break-all">
                  ...
                  {translateUi("本地路径：")}{previewAsset.localPath}
                </div>
              ) : null}
            </>
          ) : null}
  ...
  return (
    <div className="min-w-0 bg-background px-4 py-3">
      ...
      <div className="mt-1 line-clamp-3 text-sm leading-6 text-foreground">
        {props.value?.trim() || translateUi("待补充")}
      </div>
    </div>
  );
}
