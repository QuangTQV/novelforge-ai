            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-2 rounded-md border p-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-sm font-medium">{t("advanced.title")}</div>
                ...
                <div className="grid gap-2 md:grid-cols-2">
                  <label className="space-y-1 text-sm">
                    <div className="text-xs text-muted-foreground">{t("advanced.storyFunction")}</div>
                    <SelectControl
                    ...
                    >
                      ...
                      <option value={translateUi("主角")}>{t("options.protagonist")}</option>
                      <option value={translateUi("反派")}>{t("options.antagonist")}</option>
                      <option value={translateUi("导师")}>{t("options.mentor")}</option>
                      <option value={translateUi("对照组")}>{t("options.foil")}</option>
                      <option value={translateUi("配角")}>{t("options.supporting")}</option>
                    </SelectControl>
                  </label>

                  <label className="space-y-1 text-sm">
                    <div className="text-xs text-muted-foreground">{t("advanced.growthStage")}</div>
                    <SelectControl
                      className="h-10 w-full rounded-md border bg-background px-2 text-sm"
                    ...
                    >
                      <option value="">{t("options.unspecified")}</option>
                      <option value={translateUi("起点")}>{t("options.start")}</option>
                      <option value={translateUi("受挫")}>{t("options.setback")}</option>
                      <option value={translateUi("转折")}>{t("options.turningPoint")}</option>
                      <option value={translateUi("觉醒")}>{t("options.awakening")}</option>
                      <option value={translateUi("收束")}>{t("options.resolution")}</option>
                    </SelectControl>
                  </label>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <div className="space-y-1">
                  <div className="text-sm font-medium">{translateUi("参考知识库（可多选）")}</div>
                  <div className="max-h-48 space-y-2 overflow-auto rounded-md border p-2">
                    ...
                    {!knowledgeDocumentsQuery.isLoading && knowledgeDocuments.length === 0 ? (
                      <div className="text-sm text-muted-foreground">{translateUi("暂无可选知识文档。")}</div>
                    ) : null}
                  ...
                  <div className="text-xs text-muted-foreground">{translateUi("未选择则不引用知识库内容。")}</div>
                </div>

                <div className="space-y-1">
                  <div className="text-sm font-medium">{translateUi("参考拆书分析（可多选）")}</div>
                  <div className="max-h-48 space-y-2 overflow-auto rounded-md border p-2">
                    {bookAnalysesQuery.isLoading ? (
                      <div className="text-sm text-muted-foreground">{translateUi("加载中...")}</div>
                    ) : null}
                    {!bookAnalysesQuery.isLoading && bookAnalyses.length === 0 ? (
                      <div className="text-sm text-muted-foreground">{translateUi("暂无可选拆书分析。")}</div>
                    ) : null}
                  ...
                  <div className="text-xs text-muted-foreground">{translateUi("仅展示已完成的拆书分析。")}</div>
                </div>
              </div>

              <div className="text-xs text-muted-foreground">

                {translateUi("已选参考：知识库")} {selectedKnowledgeDocumentIds.length}  {translateUi("项，拆书")} {selectedBookAnalysisIds.length}  {translateUi("项。")}
              </div>
              <Button
                onClick={() => generateMutation.mutate()}
                disabled={generateMutation.isPending || !aiDescription.trim()}
              >
                {generateMutation.isPending ? translateUi("生成中...") : translateUi("生成并入库")}
              </Button>
            </CardContent>
          </Card>
        </div>
