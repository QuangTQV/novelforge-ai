  ...
  return (
    <div className="mt-3 rounded-md border border-border bg-muted/20 px-4 py-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="text-sm font-medium text-foreground">{translateUi("运行细节")}</div>
          <div className="mt-1 text-xs text-muted-foreground">

            {translateUi("底层执行记录 ·")} {entries.length}  {translateUi("条")}
          </div>
        </div>
        <Button
          type="button"
        ...
        >
          {expanded ? translateUi("收起细节") : translateUi("展开细节")}
        </Button>
      </div>
      {expanded ? (
        ...
        <div className="mt-2 text-xs text-muted-foreground">
          ...
          {translateUi("默认已折叠底层运行、工具与检查点细节；展开后可查看完整调试轨迹。")}
        </div>
      )}
    </div>
  );
