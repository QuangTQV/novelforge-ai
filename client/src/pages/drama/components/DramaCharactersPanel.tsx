...
function assetCompleteness(draft: DramaCharacterAssetInput) {
  const items = [
    { key: "screenRole", label: translateUi("出镜功能"), done: Boolean(draft.screenRole.trim()) },
    { key: "audienceRead", label: translateUi("观众识别"), done: Boolean(draft.audienceRead.trim()) },
    { key: "visualAnchor", label: translateUi("造型锚点"), done: Boolean(draft.visualAnchor.trim()) },
    { key: "voiceAnchor", label: translateUi("表演锚点"), done: Boolean(draft.voiceAnchor.trim()) },
    { key: "lineRule", label: translateUi("台词规则"), done: Boolean(draft.lineRule.trim()) },
  ];
  return {
    items,
    ...
    imageFlow.start({
      ...
      generate: async (overrides) => {
        ...
        try {
          ...
          setSheet(result.data ?? { status: "error", error: translateUi("无结果") });
          return result;
        } catch (error) {
          setSheet({ status: "error", error: error instanceof Error ? error.message : translateUi("生成失败") });
          throw error;
        } finally {
          setBusy(false);
        }
  ...
  return (
    <div className="space-y-3 border-t pt-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-sm font-medium">{translateUi("角色设计稿")}</p>
          <p className="text-xs text-muted-foreground">{translateUi("面部特写 + 全身正/侧/背三视图 — 生成后锁定跨集视觉一致性")}</p>
        </div>
        {imageProviders.length > 0 ? (
          ...
          <span className="text-xs text-destructive">{translateUi("请先在设置中配置图片生成 Provider")}</span>
        )}
      </div>
      ...
      {sheet.status === "done" && sheet.url ? (
        <a href={sheet.url} target="_blank" rel="noreferrer" className="block">
          <img
            src={sheet.url}
            alt={translateUi("{{value0}} 角色设计稿", { value0: props.character.name })}
        ...
        >
          {sheet.status === "generating" ? (
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs">{translateUi("正在生成角色设计稿...")}</span>
            </div>
          ) : sheet.status === "error" ? (
            <div className="flex flex-col items-center gap-1 px-4 text-center">
              <ImageIcon className="h-5 w-5 text-destructive" />
              <span className="text-xs text-destructive">{sheet.error ?? translateUi("生成失败")}</span>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-1">
              <ImageIcon className="h-6 w-6" />
              <span className="text-xs">{translateUi("尚未生成")}</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          ...
          {sheet.status === "done" ? translateUi("重新生成设计稿") : translateUi("生成角色设计稿")}
        </Button>
        {sheet.status === "done" && (
          <>
            <Badge variant="outline">v{sheet.version ?? 1}</Badge>
            <span className="text-xs text-muted-foreground">{translateUi("视频生成将自动引用此图作为角色参考")}</span>
          </>
        )}
        {sheet.history?.length ? <Badge variant="secondary">{sheet.history.length}  {translateUi("个历史版本")}</Badge> : null}
      </div>
...
function CharacterImageHistory({ history }: { history: NonNullable<DramaCharacterPortraitData["history"]> }) {
  ...
  return (
    <div className="rounded-md border border-dashed p-3 text-xs">
      <div className="mb-2 font-medium">{translateUi("设计稿历史版本")}</div>
      ...
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-2">
            ...
            <CardDescription>{draft.screenRole || translateUi("先明确这个角色在短剧里的出镜功能。")}</CardDescription>
          </div>
          <Badge variant={completeness.done >= 4 ? "default" : "secondary"}>

            {translateUi("资产")} {completeness.done}/{completeness.total}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="block space-y-1.5 text-sm">
            <span className="font-medium">{translateUi("出镜名")}</span>
            <input
          ...
          <label className="block space-y-1.5 text-sm">
            <span className="font-medium">{translateUi("短剧功能")}</span>
            <input
              className="h-10 w-full rounded-md border bg-background px-3 text-sm"
              value={draft.screenRole}
              placeholder={translateUi("主角 / 反派 / 阻力角色 / 助攻 / 情感对象")}
        ...
        <label className="block space-y-1.5 text-sm">
          <span className="font-medium">{translateUi("观众一眼要看懂什么")}</span>
          <textarea
            className="min-h-20 w-full rounded-md border bg-background px-3 py-2 text-sm"
            value={draft.audienceRead}
            placeholder={translateUi("例如：她表面低位受辱，但眼神始终冷静，观众要立刻相信她有反击底牌。")}
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <label className="block space-y-1.5 text-sm">
            ...
            <textarea
              ...
              placeholder={translateUi("外貌、发型、服装、随身物、常用色彩；后续分镜和视频提示词会沿用。")}
          ...
          <label className="block space-y-1.5 text-sm">
            ...
            <textarea
              ...
              placeholder={translateUi("语速、音色、表情习惯、情绪爆发方式；用于台词和视频表演一致。")}
        ...
        <label className="block space-y-1.5 text-sm">
          <span className="font-medium">{translateUi("台词规则")}</span>
          <textarea
            className="min-h-20 w-full rounded-md border bg-background px-3 py-2 text-sm"
            value={draft.lineRule}
            placeholder={translateUi("例如：短句压迫、少解释、多反问；被羞辱时不急着辩解，反击时一句话落锤。")}
            onChange={(event) => setDraft((current) => ({ ...current, lineRule: event.target.value }))}
          />
        </label>

        <label className="block space-y-1.5 text-sm">
          <span className="font-medium">{translateUi("冲突关系和镜头搭配")}</span>
          <textarea
            className="min-h-20 w-full rounded-md border bg-background px-3 py-2 text-sm"
            value={draft.relationMap}
            placeholder={translateUi("这个角色主要和谁对戏、压制谁、保护谁，适合怎样同框。")}
...
export function DramaCharactersPanel(props: {
  ...
  return (
    <div className="space-y-4">
      <Card className="rounded-lg">
        <CardHeader>
          <CardTitle className="text-lg">{translateUi("短剧角色资产")}</CardTitle>
          <CardDescription>{translateUi("角色资产会进入台本、分镜和视频提示词，优先保证观众识别、造型一致和台词可拍。")}</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-wrap gap-2">
          ...
          >
            <option value="" disabled>{props.library.length > 0 ? translateUi("选择短剧角色资产") : translateUi("暂无可导入角色资产")}</option>
      ...
      {characters.length === 0 ? (
        <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">{translateUi("整理素材后会自动导入主要角色，再补齐造型、表演和台词锚点。")}</div>
      ) : (
