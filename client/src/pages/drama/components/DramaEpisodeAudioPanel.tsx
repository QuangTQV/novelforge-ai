  ...
  return (
    <section className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-medium">{translateUi("配音")}</h3>
        <div className="flex flex-wrap gap-2">
          <SelectControl
            className="h-9 rounded-md border bg-background px-2 text-xs"
            value={activeProvider}
            onChange={(event) => setSelectedProvider(event.target.value)}
            aria-label={translateUi("配音通道")}
          >
            {props.ttsProviders.length > 0 ? props.ttsProviders.map((provider) => (
              ...
              <option value="mock">{translateUi("模拟配音通道")}</option>
            )}
          </SelectControl>
          <Button
            size="sm"
      ...
      {latestTtsBatch ? (
        <div className="rounded-md border p-3 text-sm">
          <div className="flex items-center justify-between gap-3">
            <div className="font-medium">{translateUi("本集配音任务")}</div>
          ...
          <div className="mt-2 flex flex-wrap gap-3 text-xs text-muted-foreground">
            ...
            {latestProgress.skipped ? <span>{translateUi("已跳过")} {latestProgress.skipped}</span> : null}
            {latestProgress.failed ? <span>{translateUi("失败")} {latestProgress.failed}</span> : null}
            {latestProgress.provider ? <span>{translateUi("通道：")}{latestProgress.provider}</span> : null}
            {latestProgress.cost ? <span>{translateUi("预计：")}{formatCost(latestProgress.cost, latestProgress.cost.estimated)}</span> : null}
            {latestProgress.cost ? <span>{translateUi("实际：")}{formatCost(latestProgress.cost, latestProgress.cost.actual)}</span> : null}
          </div>
          {failedShotIds.length > 0 ? (
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <span className="text-xs text-destructive">{translateUi("失败镜头：")}{failedShotIds.join(translateUi("、"))}</span>
              <Button
                size="sm"
                type="button"
                variant="outline"
      ...
      {audioItems.length > 0 ? (
          ...
          {audioItems.map((item) => (
            <div key={`${item.shotOrder}-${item.lineIndex}`} className="rounded-md border p-3 text-sm">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <Badge variant="secondary">{translateUi("镜头")} {item.shotOrder}</Badge>
                {item.speaker ? <span className="font-medium">{item.speaker}</span> : null}
                {item.voiceId ? <span className="text-xs text-muted-foreground">{translateUi("声线：")}{item.voiceId}</span> : null}
              </div>
        ...
        <div className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">{translateUi("生成分镜后可合成本集配音。")}</div>
      )}
    </section>
  );
}
...
function CostEstimate(props: { cost?: DramaBatchCostBreakdown; loading: boolean }) {
  return (
    <div className="rounded-md border border-dashed p-3 text-sm">
      <div className="text-xs text-muted-foreground">{translateUi("配音预计费用")}</div>
      <div className="mt-1 font-medium">
        {props.loading ? translateUi("计算中") : props.cost ? formatCost(props.cost, props.cost.estimated) : translateUi("生成分镜后可计算")}
      </div>
      {props.cost ? (
        <div className="mt-1 text-xs text-muted-foreground">
          {props.cost.unit.costPerSecond ? translateUi("时长 {{value0}}/秒", { value0: formatCost(props.cost, props.cost.unit.costPerSecond) }) : translateUi("未配置单价")}
