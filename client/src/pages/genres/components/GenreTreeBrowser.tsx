  ...
  return (
    <div className="grid overflow-hidden rounded-lg border border-border/80 bg-background lg:grid-cols-[320px_minmax(0,1fr)]">
      <AssetTreeNavigator
        ...
        hint={translateUi("点击节点查看详情")}
        ariaLabel={translateUi("题材基底树")}
      ...
      <section className="flex min-w-0 flex-col" aria-labelledby="selected-genre-title">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/70 px-5 py-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              title={deleteDisabled ? translateUi("当前题材或下级题材仍被小说使用，请先调整关联作品。") : undefined}
        ...
        <div className="px-5 py-5 sm:px-7 sm:py-6">
          <div className="max-w-3xl">
            <div className="text-xs font-medium tracking-[0.16em] text-muted-foreground">{translateUi("题材基底")}</div>
            <h2 id="selected-genre-title" className="mt-2 text-2xl font-semibold tracking-tight text-foreground">
              {translateUi(selectedNode.name)}
            </h2>
            <p className="mt-4 text-sm leading-7 text-muted-foreground">
              {selectedNode.description?.trim() ? translateUi(selectedNode.description.trim()) : translateUi("尚未说明这个题材面向什么读者、承诺什么体验。补充说明后，AI 能更准确地用于开书和规划。")}
            </p>

            <div className="mt-5 flex flex-wrap gap-x-6 gap-y-2 border-y border-border/70 py-3 text-xs text-muted-foreground">
              <span>{translateUi("直接关联")} <strong className="ml-1 font-semibold text-foreground">{selectedNode.novelCount}</strong></span>
              <span>{translateUi("下级题材")} <strong className="ml-1 font-semibold text-foreground">{selectedNode.childCount}</strong></span>
              <span>{translateUi("分支作品")} <strong className="ml-1 font-semibold text-foreground">{boundNovelCount}</strong></span>
            </div>
