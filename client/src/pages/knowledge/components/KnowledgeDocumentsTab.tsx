...
export default function KnowledgeDocumentsTab({
  ...
  const statusOptions = [
    { value: "", label: translateUi("全部未归档") },
    { value: "enabled", label: translateUi("仅启用") },
    { value: "disabled", label: translateUi("仅停用") },
    { value: "archived", label: translateUi("仅归档") },
  ] as const;

  const confirmArchiveDocument = (document: KnowledgeDocumentSummary) => {
    const confirmed = window.confirm(
      translateUi("确认归档“{{value0}}”吗？归档会移出默认检索和资料选择，原文与版本会保留，可在“仅归档”中恢复启用。", { value0: document.title }),
    );
    if (!confirmed) {
      return;
    }
          ...
          <div className="flex min-w-0 flex-1 items-start gap-3">
            ...
            <div className="min-w-0 space-y-1">
              ...
              <div className="text-xs text-muted-foreground">
                ...
                {translateUi("当前 v")}{document.activeVersionNumber}  {translateUi("· 共")} {document.versionCount}  {translateUi("个版本 ·")} {formatDocumentKind(document.kind)}
              </div>
              {document.bookAnalysisCount > 0 ? (
                <div className="text-xs text-muted-foreground">{translateUi("关联")} {document.bookAnalysisCount}  {translateUi("个拆书项目")}</div>
              ) : null}
            </div>
          </div>
          ...
          <details className="group mt-3">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Button asChild size="sm" variant="ghost" className="rounded-full">
                <Link to={`/book-analysis?documentId=${document.id}`}>{translateUi("新建拆书")}</Link>
              </Button>
              {document.kind === "analysis_published" && document.sourceAnalysisId ? (
                <Button asChild size="sm" variant="ghost" className="rounded-full">
                  <Link to={`/book-analysis?analysisId=${document.sourceAnalysisId}`}>{translateUi("查看来源拆书")}</Link>
                </Button>
              ) : null}
              ...
              {document.status === "enabled" ? (
                <Button size="sm" variant="ghost" className="rounded-full" onClick={() => onUpdateStatus(document.id, "disabled")}>{translateUi("停用")}</Button>
              ) : document.status === "disabled" ? (
                <Button size="sm" variant="ghost" className="rounded-full" onClick={() => onUpdateStatus(document.id, "enabled")}>{translateUi("启用")}</Button>
              ) : null}
  ...
  const renderDocuments = () => {
      ...
      return (
        <div className="flex min-h-40 items-center justify-center rounded-md border border-dashed border-border px-5 py-8 text-center" role="status">
          <div>
            ...
            <p className="mt-3 text-sm font-medium text-foreground">{translateUi("正在加载创作资料")}</p>
            <p className="mt-1 text-sm text-muted-foreground">{translateUi("正在确认资料版本和索引状态。")}</p>
          </div>
        </div>
      );
    }
    ...
    if (errorMessage) {
      return (
        <AssetLibraryEmptyState
          icon={CircleAlert}
          title={translateUi("创作资料暂时无法加载")}
          description={translateUi("{{value0}} 重新加载不会修改已有资料。", { value0: errorMessage })}
          action={(
    ...
    if (documents.length === 0) {
      return (
        <AssetLibraryEmptyState
          icon={FileText}
          title={hasFilters ? translateUi("没有符合条件的资料") : translateUi("还没有创作资料")}
          description={hasFilters
            ? translateUi("调整搜索词或状态筛选，返回其他资料。")
            : translateUi("上传 TXT 资料后，系统会建立可供拆书、规划和正文创作使用的检索索引。")}
          action={hasFilters ? (
    ...
    <>
      <section className="scroll-mt-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-xl font-semibold tracking-tight text-foreground">{translateUi("资料书架")}</h2>
            <p className="mt-1 text-sm leading-6 text-muted-foreground">{translateUi("选择可检索资料继续创作，版本与索引维护按需处理。")}</p>
          </div>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={handleDialogOpenChange}>
        <AppDialogContent
          className="max-w-lg"
          title={translateUi("上传文档")}
          description={translateUi("添加可用于检索、拆书和创作参考的文本资料。")}
        >
          <div className="space-y-4">
            <Input
              value={uploadTitle}
              onChange={(event) => onUploadTitleChange(event.target.value)}
              placeholder={translateUi("可选标题，留空则使用文件名")}
            />
            ...
            <div
              ...
              aria-label={selectedFile ? undefined : translateUi("选择要上传的 TXT 文本资料")}
              className={[
              ...
              {selectedFile ? (
                <>
                  ...
                  <button
                    ...
                    aria-label={translateUi("移除已选择的文件")}
                  >
                    <X className="h-4 w-4" />
                  </button>
                </>
                ...
                <>
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-medium">
                      {dragOver ? translateUi("松开鼠标上传") : translateUi("拖拽文件到此处，或点击选择")}
                    </p>
                    <p className="text-xs text-muted-foreground">{translateUi("仅支持 .txt 文本文件")}</p>
                  </div>
                </>
              )}
            </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground leading-5">
                ...
                {translateUi("同名标题会追加为新版本并设为当前版本")}
              </p>
              <Button
                type="button"
                size="sm"
              ...
              >
                {uploadBusy ? translateUi("上传中…") : translateUi("确认上传")}
              </Button>
            </div>
          </div>
        </AppDialogContent>
