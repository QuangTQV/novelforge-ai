...
export default function KnowledgeOpsTab({
  ...
  return (
    <div className="space-y-6">
      <section
        aria-label={translateUi("资料检索可用状态")}
      ...
      >
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <h2 className="text-lg font-semibold tracking-tight">
                {healthOk ? translateUi("资料可以用于创作") : translateUi("资料检索需要处理")}
              </h2>
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                {healthOk
                  ? translateUi("向量模型与资料库连接正常，已完成索引的资料可以参与拆书、规划和正文创作。")
                  : translateUi("向量模型或资料库连接异常，修复连接后即可继续建立索引和召回资料。")}
              </p>
            </div>
          </div>
          {!healthOk ? (
        ...
        <div className="mt-5 flex flex-wrap gap-x-6 gap-y-2 border-t border-foreground/[0.06] pt-4 text-sm">
          <span><strong className="tabular-nums">{visibleDocumentsCount}</strong>  {translateUi("份资料")}</span>
          <span className="text-muted-foreground"><strong className="font-semibold tabular-nums text-foreground">{enabledCount}</strong>  {translateUi("份已启用")}</span>
          {disabledCount > 0 ? (
            <span className="text-muted-foreground"><strong className="font-semibold tabular-nums text-foreground">{disabledCount}</strong>  {translateUi("份已停用")}</span>
          ) : null}
          <span className="text-muted-foreground"><strong className="font-semibold tabular-nums text-foreground">{activeJobCount}</strong>  {translateUi("个同步中")}</span>
          {failedJobs.length > 0 ? (
            <span className="text-destructive"><strong className="font-semibold tabular-nums">{failedJobs.length}</strong>  {translateUi("个任务失败")}</span>
          ) : null}
        </div>

        <details className="group mt-4 text-xs text-muted-foreground">
          <summary className="cursor-pointer list-none marker:hidden">{translateUi("查看连接详情")}</summary>
          <div className="mt-3 grid gap-3 rounded-2xl bg-background/55 p-4 sm:grid-cols-2">
            <div>
              <div className="font-medium text-foreground">{translateUi("向量模型")}</div>
              <div className="mt-1 break-words">
                {ragHealth?.embedding.provider ?? "-"} · {ragHealth?.embedding.model ?? "-"} · {ragHealth?.embedding.ok ? translateUi("可用") : translateUi("不可用")}
              </div>
              {ragHealth?.embedding.detail ? <div className="mt-1 break-words">{ragHealth.embedding.detail}</div> : null}
            </div>
            <div>
              <div className="font-medium text-foreground">{translateUi("资料库连接")}</div>
              <div className="mt-1">{ragHealth?.qdrant.ok ? translateUi("连接正常") : translateUi("连接失败")}</div>
      ...
      <section aria-labelledby="knowledge-jobs-title">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 id="knowledge-jobs-title" className="text-xl font-semibold tracking-tight">{translateUi("资料同步记录")}</h2>
            <p className="mt-1 text-sm leading-6 text-muted-foreground">{translateUi("查看资料进入检索库的进度；失败原因会直接显示，运行细节按需展开。")}</p>
          </div>
          <Button
            type="button"
            variant="ghost"
          ...
          >
            ...
            {isClearingJobs ? translateUi("清理中...") : translateUi("清理记录 {{value0}}", { value0: finishedJobCount })}
          </Button>
        </div>
        ...
        {jobs.length === 0 ? (
          <div className="mt-5 flex min-h-44 flex-col items-center justify-center rounded-3xl bg-muted/20 px-6 text-center">
            ...
            <div className="mt-3 font-medium">{translateUi("还没有资料同步记录")}</div>
            <div className="mt-1 text-sm text-muted-foreground">{translateUi("上传资料或重建索引后，可以在这里查看进度。")}</div>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3 border-t border-border/30 pt-3">
                    <details className="group min-w-0 text-xs text-muted-foreground">
                      <summary className="cursor-pointer list-none marker:hidden">{translateUi("任务详情")}</summary>
                    ...
                    {canDeleteRagJob(job) ? (
                      <Button
                        ...
                        aria-label={translateUi("删除任务记录")}
                      >
                        <Trash2 className="h-4 w-4" />
                        {deletingJobId === job.id ? translateUi("删除中...") : translateUi("删除")}
                      </Button>
                    ) : null}
                  </div>
                </article>
