    ...
    <div className="mx-auto max-w-5xl space-y-7 px-3 py-4 sm:px-4 lg:px-0">
      <section className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div className="max-w-3xl">
          <h1 className="text-3xl font-semibold tracking-normal text-foreground">{translateUi("创建小说项目")}</h1>
          <p className="mt-3 text-sm leading-6 text-muted-foreground">

            {translateUi("推荐先让 AI 自动导演从一句灵感整理方向、世界、角色和章节准备。需要完全手动填写时，也可以继续使用下方表单。")}
          </p>
        </div>
        <Button type="button" asChild className="shrink-0">
          <Link to="/novels/auto-director">{translateUi("AI 自动导演开书")}</Link>
        </Button>
      </section>

      <section className="space-y-4">
        <div>
          <div className="text-lg font-semibold leading-7 text-foreground">{translateUi("手动创建")}</div>
          <div className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">

            {translateUi("手动路径适合你已经清楚题材、卖点和前期承诺的项目；创建后仍可在工作台继续调整。")}
          </div>
        </div>
        <NovelBasicInfoForm
          basicForm={basicForm}
          ...
          submitLabel={translateUi("创建并进入项目")}
