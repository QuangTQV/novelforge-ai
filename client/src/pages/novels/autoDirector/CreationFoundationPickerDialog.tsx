      ...
      >
        {loading ? (
          <div className="flex min-h-72 items-center justify-center text-sm text-muted-foreground">
            <Loader2 className="mr-2 h-4 w-4 animate-spin" />  {translateUi("正在读取可选方向")}
          </div>
        ) : error ? (
          <div className="flex min-h-72 flex-col items-center justify-center gap-3 px-6 text-center">
            <div className="text-sm font-medium text-foreground">{translateUi("暂时无法读取可选方向")}</div>
            <div className="text-xs leading-5 text-muted-foreground">{translateUi("你仍可交给 AI 自动搭配，或重新加载后再选择。")}</div>
            <Button type="button" variant="outline" size="sm" onClick={onRetry}>{translateUi("重新加载")}</Button>
          </div>
        ) : nodes.length === 0 ? (
            ...
            <div className="border-b border-border/70 lg:border-b-0 lg:border-r">
              <div className="border-b border-border/70 p-3">
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="h-9 pl-9"
                    placeholder={translateUi("搜索名称或说明")}
                  />
                </div>
              </div>
              {filteredNodes.length > 0 ? (
                <AssetTreeNavigator
                  ...
                  onSelect={setDraftId}
                  title={treeTitle}
                  hint={translateUi("点击查看")}
                  ariaLabel={translateUi("{{value0}}目录", { value0: title })}
                  viewportClassName="max-h-[356px]"
                />
              ) : (
                <div className="px-4 py-10 text-center text-sm text-muted-foreground">{translateUi("没有匹配的方向")}</div>
              )}
            </div>
              ...
              {selectedNode ? (
                renderDetails ? renderDetails(selectedNode) : (
                  <div className="flex min-h-full flex-col justify-center">
                    <div className="text-xs font-medium tracking-[0.16em] text-muted-foreground">{translateUi("当前选择")}</div>
                    <div className="mt-3 text-2xl font-semibold text-foreground">{translateUi(selectedNode.name)}</div>
                    <p className="mt-4 max-w-xl text-sm leading-7 text-muted-foreground">
                      {selectedNode.description?.trim() ? translateUi(selectedNode.description.trim()) : translateUi("这个方向会作为 AI 整理整本书方案时的创作依据。")}
                    </p>
                  </div>
                )
              ) : (
                <div className="flex min-h-full flex-col justify-center">
                  <div className="text-xs font-medium tracking-[0.16em] text-muted-foreground">{translateUi("保持轻松")}</div>
                  <div className="mt-3 text-2xl font-semibold text-foreground">{autoLabel}</div>
                  <p className="mt-4 max-w-xl text-sm leading-7 text-muted-foreground">

                    {translateUi("AI 会结合你的起始想法、目标平台和阅读感，自动选择更适合写完整本书的方向。")}
                  </p>
                </div>
              )}
            </div>
