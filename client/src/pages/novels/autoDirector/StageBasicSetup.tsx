  ...
  return (
    <section className="mx-auto w-full max-w-5xl space-y-7 py-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="text-2xl font-semibold tracking-normal text-foreground">{translateUi("先定这本书的手感")}</div>
          <div className={`mt-2 max-w-2xl text-sm leading-6 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>

            {translateUi("这里只确认影响整本书阅读感的基础参数。不确定时保持默认，AI 会继续根据你的起始想法判断。")}
          </div>
        </div>
      ...
      <div className="flex items-start gap-3 border-y border-border/60 py-4">
        ...
        <div>
          <div className="text-sm font-medium text-foreground">{translateUi("AI 会自动确定创作底座")}</div>
          <div className={`mt-1 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>

            {translateUi("系统会根据起始想法确定题材和主要推进方式，并贯穿方向规划、正文、检查与修复。创建后仍可在作品设置中调整后续写法。")}
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-x-8 gap-y-6 md:grid-cols-2">
        <div className="space-y-2">
          <FieldLabel htmlFor="director-basic-platform" hint={translateUi("决定规划、正文、审校和修复采用哪类平台读感。")}>{translateUi("目标平台")}</FieldLabel>
          <SelectControl
        ...
        <div className="space-y-2">
          <FieldLabel htmlFor="director-basic-reader-channel" hint={BASIC_INFO_FIELD_HINTS.readerChannelPreference}>{translateUi("读者频道倾向")}</FieldLabel>
          <SelectControl
        ...
        <div className="space-y-2">
          <FieldLabel htmlFor="director-basic-pov" hint={BASIC_INFO_FIELD_HINTS.narrativePov}>{translateUi("叙事视角")}</FieldLabel>
          <SelectControl
        ...
        <div className="space-y-2">
          <FieldLabel htmlFor="director-basic-pace" hint={BASIC_INFO_FIELD_HINTS.pacePreference}>{translateUi("节奏偏好")}</FieldLabel>
          <SelectControl
        ...
        <div className="space-y-2">
          <FieldLabel htmlFor="director-basic-emotion" hint={BASIC_INFO_FIELD_HINTS.emotionIntensity}>{translateUi("情绪浓度")}</FieldLabel>
          <SelectControl
        ...
        <div className="space-y-2">
          <FieldLabel htmlFor="director-basic-estimated" hint={BASIC_INFO_FIELD_HINTS.estimatedChapterCount}>{translateUi("预计章节数")}</FieldLabel>
          <Input
          ...
          <div className={`text-xs text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
            ...
            {translateUi("会作为整书结构密度和后续卷章规划的参考，不是硬性上限。")}
          </div>
      ...
      <details className="group pt-2">
        <summary className="cursor-pointer list-none">
          <div>
            <div className="text-base font-semibold text-foreground">{translateUi("补充读者与卖点")}</div>
            <div className={`mt-1 max-w-3xl text-sm leading-6 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>

              {translateUi("不确定可以先跳过。补充后，AI 会更清楚这本书写给谁、前 30 章要给读者什么。")}
            </div>
          </div>
        </summary>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={translateUi("例如：爱看都市高压逆袭、关系拉扯和持续追更钩子的读者")}
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={translateUi("例如：逆袭，强冲突，悬念拉满，职场博弈")}
          ...
          <div className="space-y-2">
            <FieldLabel htmlFor="director-basic-competing-feel" hint={BASIC_INFO_FIELD_HINTS.competingFeel}>
              ...
              {translateUi("竞品感 / 熟悉阅读感")}
            </FieldLabel>
            <Input
              id="director-basic-competing-feel"
              className={controlClassName}
              value={basicForm.competingFeel}
              placeholder={translateUi("例如：现实职场压迫感里带一点冷幽默和高密度关系拉扯")}
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder={translateUi("例如：主角每次解决现实困局都会撬动更大的关系链和利益链，读者会一直期待下一次反压。")}
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder={translateUi("例如：前 30 章必须让读者看到主角站稳第一阶段立场、核心对手浮出水面、关系线第一次强反转，并明确这本书后面会越写越狠。")}
      ...
      <div className="flex flex-col gap-2 pt-2 sm:flex-row sm:justify-between">
        <Button type="button" variant="ghost" onClick={onBack}>{translateUi("返回想法")}</Button>
        <Button type="button" onClick={onConfirm}>{translateUi("确认起始设置")}</Button>
      </div>
    </section>
  );
}
