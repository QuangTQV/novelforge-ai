  ...
  return (
    <section className="mx-auto w-full max-w-5xl space-y-7 py-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="text-2xl font-semibold tracking-normal text-foreground">{translateUi("确认模型与生产准备")}</div>
          <div className={`mt-2 max-w-2xl text-sm leading-6 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>

            {translateUi("确认后，AI 会先给出两套整书方向，再自动完成开写前的角色和卷章准备。")}
          </div>
        </div>
      ...
      <div className="space-y-6">
        <section className="overflow-hidden rounded-2xl border border-border/70 bg-background shadow-[0_20px_55px_-45px_hsl(var(--foreground)/0.5)]">
          <div className="border-b border-border/60 bg-muted/15 px-5 py-4">
            <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
              ...
              {translateUi("自动导演会这样推进")}
            </div>
          </div>
          <div className="grid gap-px bg-border/60 md:grid-cols-3">
            <div className="bg-background p-5">
              <Sparkles className="h-5 w-5 text-primary" />
              <div className="mt-3 text-sm font-semibold text-foreground">{translateUi("1. 选择整书方向")}</div>
              <div className="mt-1 text-xs leading-5 text-muted-foreground">{translateUi("AI 生成两套差异明确的方向，由你二选一。")}</div>
            </div>
            <div className="bg-background p-5">
              <BookOpen className="h-5 w-5 text-primary" />
              <div className="mt-3 text-sm font-semibold text-foreground">{translateUi("2. 自动准备到可开写")}</div>
              <div className="mt-1 text-xs leading-5 text-muted-foreground">{translateUi("系统完成角色、卷战略、拆章和章节执行资源。")}</div>
            </div>
            <div className="bg-background p-5">
              <Settings2 className="h-5 w-5 text-primary" />
              <div className="mt-3 text-sm font-semibold text-foreground">{translateUi("3. 选择正文生产方式")}</div>
              <div className="mt-1 text-xs leading-5 text-muted-foreground">{translateUi("简易创作由 AI 写完整本书；专业创作进入完整工作台。")}</div>
            </div>
          </div>
        ...
        <details className="group rounded-2xl border border-border/70 bg-background px-5 py-4">
          <summary className="cursor-pointer list-none">
            ...
            <div className={`mt-1 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
              ...
              {translateUi("调整本次开书使用的模型、温度与输出上限。结构规划建议将温度保持在 0.3～0.7。")}
            </div>
          </summary>
          <div className="mt-4">
        ...
        <section className="flex flex-col gap-3 rounded-2xl border border-border/70 bg-background px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="text-sm font-medium text-foreground">{translateUi("本书问题管理")}</div>
            <div className={`mt-1 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
              {issuePolicyLoading ? translateUi("正在读取默认规则。") : translateUi("{{value0}}；不调整也可以直接开书。", { value0: issuePolicySummary })}
            </div>
          </div>
        ...
        <Dialog open={issuePolicyDialogOpen} onOpenChange={setIssuePolicyDialogOpen}>
          <DialogContent className="max-h-[88vh] max-w-3xl overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>{translateUi("开书前确认这本书遇到问题时的处理方式；不调整即可沿用默认规则。")}</DialogDescription>
            </DialogHeader>
            <AutoDirectorIssuePolicyCard
              ...
              saveButtonLabel={translateUi("应用到本书开书任务")}
            />
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div className="flex flex-col gap-2 pt-2 sm:flex-row sm:justify-between">
        <Button type="button" variant="ghost" onClick={onBack}>{translateUi("返回世界与写法")}</Button>
        <Button type="button" onClick={onGenerate} disabled={!canGenerate}>
          {isGenerating ? translateUi("生成中...") : translateUi("开始生成方向")}
        </Button>
      </div>
    </section>
  );
