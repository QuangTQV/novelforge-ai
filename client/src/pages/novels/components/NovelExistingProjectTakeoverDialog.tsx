...
const RUN_MODE_OPTIONS: Array<{ value: DirectorRunMode; label: string; description: string }> = [
  {
    value: "auto_to_ready",
    label: translateUi("推进到可开写"),
    description: translateUi("AI 会补齐正文生产前需要的资源，再让你选择简易生产或专业生产。"),
  },
];
...
const STRATEGY_OPTIONS: Array<{ value: DirectorTakeoverStrategy; label: string; description: string }> = [
  {
    ...
    label: translateUi("继续已有进度"),
    description: translateUi("优先跳过已完成资产，只补缺失部分或恢复当前批次。"),
  },
  {
    value: "restart_current_step",
    label: translateUi("重新生成当前步"),
    description: translateUi("先清空当前步骤产出，再按该步骤重新生成。"),
  },
];
...
export default function NovelExistingProjectTakeoverDialog({
  ...
  const startMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      if (!data?.taskId) {
        toast.error(translateUi("启动自动导演失败，未返回任务信息。"));
        return;
      }
      ...
      toast.success(
        effectiveRunMode === "full_book_autopilot"
          ? translateUi("自动导演接管任务已提交，可在 AI 驾驶舱查看全书执行进度。")
          : effectiveRunMode === "auto_to_execution"
          ? translateUi("自动导演接管任务已提交，可在 AI 驾驶舱查看 {{value0}} 的执行进度。", { value0: buildDirectorAutoExecutionPlanLabel(autoExecutionPlan) })
          : translateUi("自动导演接管任务已提交，可在 AI 驾驶舱查看排队和执行进度。"),
      );
      navigate(buildEditRoute({
        novelId,
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className={AUTO_DIRECTOR_MOBILE_CLASSES.takeoverDialogContent}>
          <DialogHeader className="shrink-0 border-b px-4 pb-4 pr-12 pt-5 text-left sm:px-6 sm:pt-6">
            <DialogTitle>{translateUi("让 AI 从当前项目继续自动导演")}</DialogTitle>
            <DialogDescription>

              {translateUi("先读取当前项目真实进度，再明确告诉你这次会跳过、继续还是重跑哪些步骤。")}
            </DialogDescription>
          </DialogHeader>
          <div className={AUTO_DIRECTOR_MOBILE_CLASSES.dialogBody}>
            <div className="min-w-0 space-y-4">
              ...
              <div className="min-w-0 rounded-xl border bg-background/80 p-3 sm:p-4">
                <div className="text-sm font-medium text-foreground">{translateUi("模型设置")}</div>
                <div className="mt-3"><LLMSelector /></div>
              </div>
              <div className="min-w-0 rounded-xl border bg-background/80 p-3 sm:p-4">
                <div className="text-sm font-medium text-foreground">{translateUi("自动导演运行方式")}</div>
                <div className="mt-3 rounded-lg border bg-muted/15 p-3">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="space-y-1">
                      <div className="text-sm font-medium text-foreground">{translateUi("正文后去 AI 检测与修正")}</div>
                      <div className={`text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>

                        {translateUi("开启后，章节正文生成完成时会检测 AI 味风险，并在命中可修正问题时生成修订稿。")}
                      </div>
                    </div>
                    <Switch
                      aria-label={translateUi("正文后去 AI 检测与修正")}
                ...
                {runMode === "full_book_autopilot" ? (
                  <div className={`mt-3 rounded-md border border-primary/15 bg-primary/5 p-3 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
                    <div className="text-sm font-medium text-foreground">{translateUi("全书自动接管")}</div>
                    <div className="mt-1">

                      {translateUi("系统会以整本书为目标接管当前项目，继续补齐规划、章节执行、审校和修复。只有模型不可用、服务异常、正文保护或不可恢复风险会停下。")}
                    </div>
                  </div>
                ) : null}
              </div>
              ...
              <div className="min-w-0 rounded-xl border bg-background/80 p-3 sm:p-4">
                <div className="text-sm font-medium text-foreground">{translateUi("本次接管使用的写法")}</div>
                <div className={`mt-1 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>

                  {translateUi("绑定书级默认写法后，接管时建议沿用它。前半段导演只读取轻量摘要，避免干扰结构规划。")}
                </div>
                <div className="mt-3 space-y-3">
                  ...
                  >
                    <option value="">{translateUi("先只沿用文风关键词")}</option>
                  ...
                  {currentNovelStyleBindings.length > 0 ? (
                    <div className={`rounded-lg border bg-muted/15 p-3 text-xs leading-6 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
                      ...
                      {translateUi("当前书级默认写法：")}{currentNovelStyleBindings
                  ...
                  {selectedStyleSummary?.stageSummaryLines.length ? (
                    <div className={`rounded-lg border bg-muted/15 p-3 text-xs leading-6 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
                      ...
                      {translateUi("本阶段仅生效的写法摘要：")}{selectedStyleSummary.stageSummaryLines.join(translateUi("；"))}
                    </div>
                  ) : null}
                </div>
              </div>
              ...
              <div className="min-w-0 rounded-xl border bg-background/80 p-3 sm:p-4">
                <div className="flex items-center justify-between gap-3">
                  <div className="text-sm font-medium text-foreground">{translateUi("接续位置")}</div>
                  {readinessQuery.isLoading ? <Badge variant="outline">{translateUi("读取中")}</Badge> : null}
                </div>
                  ...
                  <>
                    <div className="mt-3 grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                      <div className="rounded-lg border bg-muted/15 p-3">
                        <div className="text-xs text-muted-foreground">{translateUi("书级规划")}</div>
                        <div className="mt-1 text-sm font-medium text-foreground">{readiness.snapshot.hasStoryMacroPlan ? translateUi("已具备") : translateUi("未具备")}</div>
                      </div>
                      <div className="rounded-lg border bg-muted/15 p-3">
                        <div className="text-xs text-muted-foreground">{translateUi("创作约束")}</div>
                        <div className="mt-1 text-sm font-medium text-foreground">{readiness.snapshot.hasBookContract ? translateUi("已具备") : translateUi("未具备")}</div>
                      </div>
                      <div className="rounded-lg border bg-muted/15 p-3">
                        <div className="text-xs text-muted-foreground">{translateUi("角色数量")}</div>
                        <div className="mt-1 text-sm font-medium text-foreground">{readiness.snapshot.characterCount}</div>
                      </div>
                      <div className="rounded-lg border bg-muted/15 p-3">
                        <div className="text-xs text-muted-foreground">{translateUi("卷 / 当前卷章节")}</div>
                    ...
                    {readiness.hasActiveTask ? (
                      <div className="mt-4 rounded-lg border border-amber-500/30 bg-amber-500/10 p-4">
                        <div className="text-sm font-medium text-foreground">{translateUi("当前已有自动导演任务")}</div>
                        <div className="mt-1 text-sm text-muted-foreground">{translateUi("为避免重复接管，请先处理当前自动导演任务。")}</div>
                      ...
                      <>
                              ...
                              >
                                <div className="flex items-center justify-between gap-2">
                                  ...
                                  <div className="flex shrink-0 flex-wrap items-center gap-2">
                                    {entry.recommended ? <Badge>{translateUi("推荐")}</Badge> : null}
                                ...
                                <div className={`mt-3 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
                                  {allowedForScope ? entry.reason : translateUi("当前范围不能从这一步开始。章节范围从节奏拆章开始，卷范围从卷战略开始。")}
                                </div>
                              </button>
                            );
                          })}
                        ...
                        {selectedEntry ? (
                          <div className="mt-4 min-w-0 rounded-xl border bg-muted/15 p-3 sm:p-4">
                            <div className="text-sm font-medium text-foreground">{translateUi("本次接管预览")}</div>
                            ...
                            {selectedPreview ? (
                              <>
                                <div className="mt-3 flex min-w-0 flex-wrap gap-2">
                                  <Badge variant="secondary" className="max-w-full whitespace-normal break-words text-left [overflow-wrap:anywhere]">{translateUi("当前页：")}{selectedEntry.label}</Badge>
                                  <Badge variant="outline" className="max-w-full whitespace-normal break-words text-left [overflow-wrap:anywhere]">{translateUi("实际接管：")}{selectedPreview.effectiveStep}</Badge>
                                  <Badge variant="outline" className="max-w-full whitespace-normal break-words text-left [overflow-wrap:anywhere]">{translateUi("执行阶段：")}{selectedPreview.effectiveStage}</Badge>
                                  {selectedPreview.usesCurrentBatch ? <Badge>{translateUi("恢复当前批次")}</Badge> : null}
                                </div>
                                ...
                                {readiness.latestCheckpoint?.checkpointType ? (
                                  <div className={`mt-2 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
                                    ...
                                    {translateUi("最近检查点：")}{readiness.latestCheckpoint.checkpointType}
                                    {readiness.latestCheckpoint.chapterOrder ? translateUi(" · 第{{value0}}章", { value0: readiness.latestCheckpoint.chapterOrder }) : ""}
                                  </div>
                                ) : null}
                                {readiness.executableRange ? (
                                  <div className={`mt-2 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>

                                    {translateUi("当前可执行范围：第")} {readiness.executableRange.startOrder}-{readiness.executableRange.endOrder}  {translateUi("章")}
                                    {readiness.executableRange.nextChapterOrder ? translateUi(" · 下一章第 {{value0}} 章", { value0: readiness.executableRange.nextChapterOrder }) : ""}
                                  </div>
                                ) : null}
                                {selectedPreview.skipSteps.length > 0 ? (
                                  <div className={`mt-3 text-xs leading-5 text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>{translateUi("会跳过：")}{selectedPreview.skipSteps.join(" / ")}</div>
                                ) : null}
                        ...
                        <div className={AUTO_DIRECTOR_MOBILE_CLASSES.takeoverSubmitBar}>
                          ...
                          >
                            {startMutation.isPending ? translateUi("启动中...") : translateUi("按高级设置启动")}
                          </Button>
                        </div>
                      </>
                    )}
