  ...
  const mutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      toast.success(response.experience === "simple"
        ? translateUi("已切换到阅读书架，AI 会继续完成整本书。")
        : translateUi("已切换到完整工作台，AI 会继续完成整本书。"));
      navigate(response.targetRoute, { replace: true });
    },
    onError: (error) => toast.error(error instanceof Error ? error.message : translateUi("选择生产方式失败，请重试。")),
  });
  ...
  return (
    <section className="mx-auto max-w-5xl space-y-5 px-3 py-6 sm:px-4 lg:px-0">
      <OnboardingTip
        ...
        title={translateUi("选择你想使用的创作界面")}
        description={translateUi("两种界面共享同一套创作、审校和恢复能力；阅读书架更专注于正文，完整工作台会展示更多创作资料。")}
      />
      <div className="relative overflow-hidden rounded-3xl bg-foreground px-6 py-7 text-background shadow-[0_30px_80px_-50px_hsl(var(--foreground))] sm:px-8 sm:py-9">
        ...
        <div className="relative">
          ...
          <h1 className="mt-4 max-w-3xl text-2xl font-semibold tracking-tight sm:text-3xl">
            ...
            {translateUi("选择《")}{novelTitle?.trim() || translateUi("这本小说")}{translateUi("》的创作界面")}
          </h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-background/70">

            {translateUi("故事方向、角色和卷章安排准备完毕。选择界面后，AI 会持续推进整本书；你可以随时切换界面，创作进度和操作权限保持一致。")}
          </p>
        </div>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        <article className="flex flex-col rounded-3xl border border-primary/30 bg-background p-6 shadow-[0_24px_65px_-50px_hsl(var(--primary))] ring-1 ring-primary/10 sm:p-7">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="rounded-full bg-primary px-2.5 py-1 text-xs font-medium text-primary-foreground">{translateUi("推荐新手")}</span>
          </div>
          <h2 className="mt-5 text-xl font-semibold text-foreground">{translateUi("阅读书架")}</h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">{translateUi("优先展示章节进度、已保存正文和需要关注的事项。")}</p>
          <ul className="mt-5 flex-1 space-y-3 text-sm text-foreground">
            {[translateUi("持续写完整本书"), translateUi("自动审校、修复与必要重规划"), translateUi("专注阅读已保存正文")].map((item) => (
        ...
        <article className="flex flex-col rounded-3xl border border-border/80 bg-background p-6 sm:p-7">
          ...
          <h2 className="mt-5 text-xl font-semibold text-foreground">{translateUi("完整工作台")}</h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">{translateUi("展示规划、角色、章节和任务等完整创作资料。")}</p>
          <ul className="mt-5 flex-1 space-y-3 text-sm text-foreground">
            {[translateUi("持续写完整本书"), translateUi("查看并调整全部创作资产"), translateUi("自由修改卷章规划与正文")].map((item) => (
              <li key={item} className="flex items-center gap-2.5">
