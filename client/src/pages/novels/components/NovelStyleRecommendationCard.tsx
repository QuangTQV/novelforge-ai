      ...
      <CardHeader className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-1">
            <CardTitle>{translateUi("这本书的默认写法")}</CardTitle>
            <div className="text-sm leading-7 text-muted-foreground">

              {translateUi("这里负责为当前小说选择和应用书级写法。写法资产属于资源层，应该在这里被小说消费，而不是从资产库反向决定“哪本书来用它”。")}
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button asChild type="button" variant="outline">
              <Link to="/style-engine">{translateUi("打开资产库 / 高级编辑")}</Link>
            </Button>
            <Button asChild type="button" variant="outline">
              <Link to="/style-engine?mode=imitate">{translateUi("去新建一套写法")}</Link>
            </Button>
          </div>
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,0.96fr)_minmax(0,1.04fr)]">
          <div className="space-y-4 rounded-2xl border bg-slate-50/70 p-4">
            <div className="rounded-2xl border bg-white p-4">
              <div className="text-sm font-medium text-slate-900">{translateUi("当前书级默认写法")}</div>
              {hasConfirmedBookStyle ? (
                  ...
                  {currentBindings.map((binding) => (
                    <div key={binding.id} className="rounded-xl border bg-slate-50/70 p-3">
                      ...
                      <div className="mt-1 text-xs text-slate-600">
                        ...
                        {translateUi("优先级 P")}{binding.priority}  {translateUi("/ 强度 W")}{binding.weight}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-3 text-sm leading-7 text-muted-foreground">
                  ...
                  {translateUi("这本书还没有绑定默认写法。推荐先让系统根据当前小说的目标读者、卖点和前 30 章承诺给出 2-3 套候选。")}
                </div>
              )}
            </div>

            <div className="rounded-2xl border bg-slate-950 p-4 text-white">
              <div className="text-sm font-medium">{translateUi("生效方式")}</div>
              <div className="mt-3 space-y-2 text-sm leading-7 text-slate-200">
                <div>{translateUi("规划期：自动导演和章节规划只读取“读感承诺 / 语言密度 / 对白风格 / 情绪外显 / 反 AI 摘要”。")}</div>
                <div>{translateUi("正文期：绑定成功后，planner 和 runtime 会继续使用完整写法规则与反 AI 约束。")}</div>
              </div>
            </div>

            <div className="flex flex-wrap justify-end gap-2">
              <AiButton onClick={() => recommendMutation.mutate()} disabled={recommendMutation.isPending}>
                {recommendMutation.isPending ? translateUi("正在推荐写法...") : translateUi("生成 2-3 套写法推荐")}
              </AiButton>
              {recommendation ? (
          ...
          <div className="space-y-4 rounded-2xl border bg-white p-4">
            <div className="text-sm font-medium text-slate-900">{translateUi("推荐结果")}</div>
            {recommendation ? (
              <>
                ...
                {recommendation.candidates.length > 0 ? (
                    ...
                    {recommendation.candidates.map((candidate) => (
                      <div key={candidate.styleProfileId} className="rounded-2xl border bg-white p-4 shadow-sm">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <Badge variant="outline">{translateUi("适配度")} {candidate.fitScore}</Badge>
                        </div>
                        ...
                        {candidate.caution ? (
                          <div className="mt-3 rounded-xl border bg-amber-50/70 p-3 text-xs leading-6 text-amber-900">
                            ...
                            {translateUi("注意事项：")}{candidate.caution}
                          </div>
                        ) : null}
                        <div className="mt-4 flex justify-end">
                          ...
                          >
                            {applyMutation.isPending ? translateUi("正在绑定...") : translateUi("设为本书默认写法")}
                          </Button>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
                    ...
                    {translateUi("当前还没有可推荐的写法资产。可以先去写法引擎沉淀 1-2 套，再回来为这本书做选择。")}
                  </div>
                )}
              </>
            ) : (
              <div className="rounded-xl border border-dashed p-4 text-sm leading-7 text-muted-foreground">
                ...
                {translateUi("这里会展示系统为当前小说挑出的 2-3 套写法方案，并告诉你为什么适合、有哪些注意事项。")}
              </div>
            )}
          </div>
        </div>
