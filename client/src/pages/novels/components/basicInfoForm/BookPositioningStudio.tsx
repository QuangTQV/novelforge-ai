...
const POSITIONING_FIELDS = [
  { key: "title", label: translateUi("标题") },
  { key: "description", label: translateUi("概述") },
  { key: "targetAudience", label: translateUi("读者") },
  { key: "bookSellingPoint", label: translateUi("卖点") },
  { key: "first30ChapterPromise", label: translateUi("前 30 章") },
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs font-medium text-muted-foreground">{translateUi("标题、卖点和前期回报")}</span>
          </div>
          <h2 className="mt-3 text-lg font-semibold leading-7 text-foreground">{translateUi("把这本书的读者承诺定清楚")}</h2>
          <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">

            {translateUi("写清谁会追、为什么追、前 30 章能看到什么；AI 会沿着这组承诺继续规划世界、角色和章节。")}
          </p>
        </div>
      ...
      <div className="grid gap-5 rounded-xl bg-background/70 px-4 py-4 xl:grid-cols-[14rem_minmax(0,1fr)] xl:items-center">
        <div className="space-y-3">
          <div className="flex items-end justify-between gap-3">
            <div>
              <div className="text-xs font-medium text-muted-foreground">{translateUi("定位完成度")}</div>
              <div className="mt-1 text-xs text-muted-foreground">{completedCount} / {POSITIONING_FIELDS.length}  {translateUi("项已就绪")}</div>
            </div>
      ...
      <div className="grid items-start gap-x-8 gap-y-6 xl:grid-cols-[minmax(18rem,0.8fr)_minmax(0,1.2fr)]">
        <div className="space-y-5">
          <div className="flex min-h-16 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h3 className="text-sm font-semibold text-foreground">{translateUi("书名与故事入口")}</h3>
              <p className="mt-1 text-sm leading-6 text-muted-foreground">{translateUi("先用几句话交代主角、困境和最值得继续看的转折。")}</p>
            </div>
            {titleQuickFill ? <div className="shrink-0">{titleQuickFill}</div> : null}
          </div>

          <div className="space-y-2">
            <FieldLabel htmlFor="basic-title">{translateUi("小说标题")}</FieldLabel>
            <Input
              id="basic-title"
              value={basicForm.title}
              placeholder={translateUi("例如：雾港审判局")}
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <FieldLabel htmlFor="basic-description">{translateUi("一句话概述")}</FieldLabel>
              <span className="text-xs text-muted-foreground">{translateUi("主角 · 目标 · 阻力")}</span>
            </div>
            <textarea
              ...
              placeholder={translateUi("用 2-4 句话说明主角、核心冲突和故事看点。")}
