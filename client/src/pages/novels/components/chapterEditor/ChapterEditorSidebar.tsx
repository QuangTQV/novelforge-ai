      ...
      <div className="flex h-full min-h-0 flex-col gap-4 overflow-y-auto pr-1">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="space-y-3">
              <div className="text-lg font-semibold leading-7 text-foreground">
                ...
                {translateUi("第")} {chapter.order}  {translateUi("章 ·")} {chapter.title?.trim() || translateUi("未命名章节")}
              </div>

              <div className="flex flex-wrap gap-2">
                <MetaChip label={translateUi("{{value0}} 字", { value0: wordCount })} />
                <MetaChip label={saveStatusLabel} />
                <MetaChip label={isWorkspaceLoading ? translateUi("LLM 分析中") : translateUi("问题 {{value0}}", { value0: workspace?.chapterMeta.openIssueCount ?? 0 })} />
              </div>
              ...
              {isWorkspaceLoading ? (
                ...
                <div className="text-sm leading-6 text-muted-foreground">
                  ...
                  {translateUi("当前写法资产：")}{workspace.chapterMeta.styleSummary}
                </div>
              ) : null}
            </div>
            ...
            <div className="grid grid-cols-1 gap-2">
              <Button
                ...
                className="w-full"
              >
                {isSaving ? translateUi("保存中...") : translateUi("保存")}
              </Button>
              {onOpenVersionHistory ? (
        ...
        <div className="shrink-0 rounded-3xl border border-border/70 bg-background p-4 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div className="text-sm font-medium text-foreground">{translateUi("宏观定位")}</div>
            <span className="text-xs text-muted-foreground">
              {isWorkspaceLoading ? translateUi("AI 分析中") : workspace?.refreshReason ?? translateUi("实时生成")}
            </span>
          </div>

          {isWorkspaceLoading ? (
            <div className="space-y-4 text-sm leading-6 text-muted-foreground">
              <div>{translateUi("AI 正在分析本章在卷内的位置、节奏建议和章节任务。")}</div>
              <div className="space-y-3">
            ...
            <div className="space-y-4 text-sm leading-6">
              <div>
                <div className="mb-1 font-medium text-foreground">{translateUi("本章在本卷中的位置")}</div>
                <div className="text-muted-foreground">
                  {macroContext.volumeTitle} · {macroContext.volumePositionLabel} · {macroContext.volumePhaseLabel}
                </div>
              </div>
              <div>
                <div className="mb-1 font-medium text-foreground">{translateUi("当前节奏建议")}</div>
                <div className="text-muted-foreground">{macroContext.paceDirective}</div>
              </div>
              <div>
                <div className="mb-1 font-medium text-foreground">{translateUi("本章主要任务")}</div>
                <div className="text-muted-foreground">{macroContext.chapterMission}</div>
              </div>
              <div>
                <div className="mb-1 font-medium text-foreground">{translateUi("前后章节衔接")}</div>
                <div className="space-y-2 text-muted-foreground">
                  <div>{translateUi("承接上一章：")}{macroContext.previousChapterBridge}</div>
                  <div>{translateUi("铺向下一章：")}{macroContext.nextChapterBridge}</div>
                </div>
              </div>
            </div>
          ) : isWorkspaceError ? (
            <div className="text-sm leading-6 text-muted-foreground">
              ...
              {translateUi("宏观定位暂时加载失败，你仍然可以先编辑正文或在右侧直接发起 AI 修正。")}
            </div>
          ) : (
            <div className="text-sm leading-6 text-muted-foreground">

              {translateUi("正在准备本章的卷内定位和节奏建议。")}
            </div>
          )}
        </div>
        ...
        <div className="min-h-0 shrink-0 rounded-3xl border border-border/70 bg-background p-4 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div className="text-sm font-medium text-foreground">{translateUi("待处理问题卡")}</div>
            <span className="text-xs text-muted-foreground">
              {isWorkspaceLoading
                ? translateUi("AI 正在梳理")
                : recommendedTask
                  ? translateUi("当前推荐：{{value0}}", { value0: recommendedTask.title })
                  : translateUi("等待问题卡")}
            </span>
          </div>
            ...
            {isWorkspaceLoading ? (
              <>
                <div className="rounded-2xl border border-dashed border-border/70 bg-muted/10 p-4 text-sm leading-6 text-muted-foreground">
                  ...
                  {translateUi("AI 正在按章节问题、卷内位置和节奏目标梳理优先修正项，请稍候。")}
                </div>
                {[0, 1].map((item) => (
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-muted-foreground">
                        {card.paragraphLabel || translateUi("整章")} · {card.severity}
                      </div>
                    </div>
                    {isRecommended ? (
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    >
                      {isSelected ? translateUi("取消定位") : translateUi("定位到正文")}
                    </Button>
                    <Button size="sm" onClick={() => onRunDiagnostic(card)}>

                      {translateUi("直接用 AI 处理")}
                    </Button>
                  </div>
                </div>
              );
              ...
              <div className="rounded-2xl border border-dashed border-border/70 bg-muted/10 p-4 text-sm leading-6 text-muted-foreground">
                {isWorkspaceError
                  ? translateUi("问题卡暂时加载失败，你可以先在右侧直接输入修改意见，或手动选中片段发起修正。")
                  : workspace
                  ? translateUi("AI 暂时还没有整理出明确的问题卡，你可以先在右侧直接输入修改意见，或手动选中片段发起修正。")
                  : translateUi("正在加载本章工作区。")}
              </div>
            )}
          </div>
        </div>
