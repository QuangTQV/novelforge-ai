    ...
    <div className="space-y-3">
      <div className="rounded-xl border border-border/70 bg-background p-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-xs font-medium text-muted-foreground">{translateUi("资源与风险")}</div>
            <div className="mt-1 text-sm font-medium text-foreground">{translateUi("本章关键资源和风险提示")}</div>
            <div className="mt-1 text-xs leading-5 text-muted-foreground">{modeHint}</div>
          </div>
          <div className="flex flex-col items-end gap-2">
            <Badge variant={isAutoDirectorMode ? "secondary" : "outline"}>{isAutoDirectorMode ? translateUi("自动同步") : translateUi("手动复查")}</Badge>
        ...
        {!isAutoDirectorMode ? (
          ...
          >
            ...
            {isExtractingChapterResources ? translateUi("复查中...") : translateUi("复查本章资源")}
          </Button>
        ) : null}
      </div>
      ...
      <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-1">
        <div className="rounded-xl border border-border/70 bg-background p-3">
          <div className="text-xs font-medium text-muted-foreground">{translateUi("风险摘要")}</div>
          <div className="mt-2 space-y-2 text-xs leading-5 text-muted-foreground">
            <div className="flex items-center justify-between gap-2">
              <span>{translateUi("失败分类")}</span>
              <span className="font-medium text-foreground">{failureSummary || translateUi("无")}</span>
            </div>
            <div className="flex items-center justify-between gap-2">
              <span>{translateUi("开放冲突")}</span>
              <span className="font-medium text-foreground">{openConflicts.length}</span>
            </div>
            <div className="flex items-center justify-between gap-2">
              <span>{translateUi("阻断问题")}</span>
              <span className="font-medium text-foreground">{blockingIssues.length}</span>
            </div>
            <div className="flex items-center justify-between gap-2">
              <span>{translateUi("时间线")}</span>
              <span className="font-medium text-foreground">{timelineCheck ? getTimelineCheckLabel(timelineCheck.status) : translateUi("未检测")}</span>
            </div>
          </div>
        </div>

        <div className="rounded-xl border border-border/70 bg-background p-3">
          <div className="text-xs font-medium text-muted-foreground">{translateUi("当前章节")}</div>
          <div className="mt-2 text-sm font-medium text-foreground">
            {selectedChapter ? translateUi("第{{value0}}章 {{value1}}", { value0: selectedChapter.order, value1: selectedChapter.title || translateUi("未命名章节") }) : translateUi("未选择章节")}
          </div>
          <div className="mt-1 text-xs leading-5 text-muted-foreground">
            {chapterResourceContext?.summary ?? translateUi("选中章节后会提示本章可用、需铺垫和不可直接使用的资源。")}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <ResourceGroup title={translateUi("可用资源")} items={chapterResourceContext?.availableItems ?? []} emptyText={translateUi("没有需要特别依赖的可用资源。")} />
        <ResourceGroup title={translateUi("需要铺垫")} items={chapterResourceContext?.setupNeededItems ?? []} emptyText={translateUi("没有必须先铺垫的资源。")} />
        <ResourceGroup title={translateUi("不能提前使用")} items={chapterResourceContext?.blockedItems ?? []} emptyText={translateUi("没有被消耗、丢失或毁坏的关键资源。")} />
        <ResourceGroup title={translateUi("高风险已入账")} items={chapterResourceContext?.highRiskCommittedItems ?? []} emptyText={translateUi("没有需要谨慎使用的高风险已入账资源。")} />

        {pendingCharacterResourceProposals.length > 0 ? (
          <div className="space-y-2 rounded-lg border border-border/70 bg-muted/10 p-3">
            <div className="text-xs font-medium text-muted-foreground">{translateUi("需要判断的资源变更")}</div>
            {pendingCharacterResourceProposals.slice(0, 2).map((proposal) => (
              <div key={proposal.id} className="space-y-2 rounded-md border border-border/70 bg-background p-2">
                <div className="flex flex-wrap items-start gap-2">
                  ...
                  <Badge variant="outline">{proposal.sourceType === "chapter_background_sync" ? translateUi("自动同步发现") : translateUi("手动复查发现")}</Badge>
                </div>
                {proposal.evidence[0] ? <div className="line-clamp-2 text-[11px] leading-5 text-muted-foreground">{translateUi("证据：")}{proposal.evidence[0]}</div> : null}
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" onClick={() => onConfirmCharacterResourceProposal?.(proposal.id)} disabled={confirmingCharacterResourceProposalId === proposal.id}>
                    {confirmingCharacterResourceProposalId === proposal.id ? translateUi("确认中...") : translateUi("确认")}
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                  ...
                  >
                    {rejectingCharacterResourceProposalId === proposal.id ? translateUi("处理中...") : translateUi("忽略")}
                  </Button>
                </div>
              </div>
            ))}
