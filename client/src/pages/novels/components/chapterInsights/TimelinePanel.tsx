    ...
    return (
      <div className="rounded-xl border border-border/70 bg-muted/20 p-3 text-xs leading-6 text-muted-foreground">
        <div className="flex items-center gap-2 font-medium text-foreground">
          ...
          {translateUi("时间线检测读取中")}
        </div>
        <div className="mt-1">{translateUi("章节切换后会在这里显示最新检测结果。")}</div>
      </div>
    );
  }
  ...
  if (!hasChapter) {
    return (
      <div className="rounded-xl border border-dashed border-border/70 bg-muted/10 p-3 text-xs leading-6 text-muted-foreground">
        ...
        {translateUi("选中章节后，这里显示本章时间线检测结果。")}
      </div>
    );
  }
  ...
  return (
    <div className={cn("rounded-xl border p-3 text-sm", getTimelineCheckTone(timelineCheck.status))}>
      <div className="flex items-center justify-between gap-2">
        ...
        <Badge variant={getTimelineCheckBadgeVariant(timelineCheck.status)}>{translateUi("分数")} {Math.round(timelineCheck.score * 100)}</Badge>
      </div>
      {timelineCheck.issues.length > 0 ? (
        ...
        <div className="mt-2 text-xs leading-6 opacity-80">{translateUi("本章未见明显未来泄漏、钩子断接或关键状态冲突。")}</div>
      )}
    </div>
  );
}
    ...
    <div className="space-y-3">
      <div className="grid gap-2 md:grid-cols-2">
        ...
        <div className="rounded-xl border border-border/70 bg-background p-3">
          <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
            ...
            <span>{translateUi("检测结果")}</span>
          </div>
          <div className="mt-2 text-sm font-medium text-foreground">
            {timelineCheck ? getTimelineCheckLabel(timelineCheck.status) : isLoadingChapterTimeline ? translateUi("读取中") : translateUi("未检测")}
          </div>
          <div className="mt-1 text-xs leading-5 text-muted-foreground">
            {timelineCheck ? translateUi("分数 {{value0}}", { value0: Math.round(timelineCheck.score * 100) }) : translateUi("章节切换后会读取最新检查结果。")}
          </div>
        </div>
      </div>
      ...
      {context ? (
        <>
          ...
          <div className="rounded-xl border border-border/70 bg-background p-3">
            ...
            {context.previousEvents.length > 0 ? (
              ...
              <div className="mt-2 text-xs leading-5 text-muted-foreground">{translateUi("没有可显示的最近事件。")}</div>
            )}
          </div>
          <div className="space-y-2">
            <div className="text-xs font-medium text-muted-foreground">{translateUi("最新时间线检测")}</div>
        ...
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/10 p-3 text-xs leading-6 text-muted-foreground">
          ...
          {translateUi("选中章节后，这里显示本章时间锚点、上一章钩子、计划推进和禁止事项。")}
        </div>
      )}
    </div>
  );
