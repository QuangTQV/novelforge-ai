    ...
    <section className={cn("flex min-h-[520px] min-w-0 flex-col overflow-hidden bg-background xl:max-h-[calc(100dvh-15rem)]", props.className)}>
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-border/60 py-3.5">
        <div>
          <div className="flex items-center gap-2 text-sm font-semibold tracking-tight">
            ...
            {translateUi("和")} {props.characterName}  {translateUi("聊聊")}
          </div>
          <p className="mt-1 text-xs leading-5 text-muted-foreground">{POLICY_DESCRIPTION[props.interactionPolicy]}</p>
        </div>
        <span className="pt-0.5 text-xs text-muted-foreground">{props.session ? translateUi("谈话进行中") : translateUi("等待开启")}</span>
      </header>

      {props.isLoading ? <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">{translateUi("正在读取谈话记录...")}</div> : null}
      {!props.isLoading && !props.session ? (
        <div className="flex flex-1 flex-col items-center justify-center px-6 text-center">
          ...
          <div className="mt-4 text-base font-semibold">{translateUi("从一句话开始")}</div>
          <p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">{translateUi("可以询问")} {props.characterName}  {translateUi("的顾虑、质疑他的选择，或聊聊他眼前的局面。")}</p>
          <AiButton className="mt-5" onClick={props.onCreate} disabled={props.isCreating}>
            <Sparkles className="mr-1.5 h-4 w-4" />{props.isCreating ? translateUi("准备谈话中...") : translateUi("开启与{{value0}}的谈话", { value0: props.characterName })}
          </AiButton>
        </div>
      ) : null}
      {props.session ? (
        <>
          <div className="min-h-0 flex-1 space-y-6 overflow-y-auto px-1 py-6 sm:px-2">
            {props.session.turns.length ? props.session.turns.map((turn) => (
              <article key={turn.id} className={turn.role === "author" ? "ml-auto max-w-[82%] rounded-lg bg-primary px-4 py-3.5 text-primary-foreground" : "mr-auto max-w-[88%] border-l-2 border-primary/35 pl-4"}>
                <div className={`mb-2 text-xs font-medium ${turn.role === "author" ? "text-primary-foreground/70" : "text-primary"}`}>{turn.role === "author" ? translateUi("你") : props.characterName}</div>
                <div className="whitespace-pre-wrap text-sm leading-7">{turn.content}</div>
                {turn.role === "character" && turn.uncertainty ? <div className="mt-4 border-t border-border/60 pt-3 text-xs leading-5 text-muted-foreground"><span className="mr-1 font-medium text-foreground/75">{translateUi("不确定之处：")}</span>{turn.uncertainty}</div> : null}
                {turn.role === "character" && turn.evidence.length > 0 ? <div className="mt-3 border-t border-border/60 pt-3 text-xs leading-5 text-muted-foreground"><span className="mr-2 font-medium text-foreground/75">{translateUi("依据")}</span>{turn.evidence.map((evidence, index) => <span key={`${evidence.sourceRef ?? evidence.label}-${index}`}>{index > 0 ? " · " : ""}{evidence.chapterOrder ? translateUi("第 {{value0}} 章 · ", { value0: evidence.chapterOrder }) : ""}{evidence.label}</span>)}</div> : null}
              </article>
            )) : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">{translateUi("说说你想和")} {props.characterName}  {translateUi("谈的事。")}</div>}
          </div>
          ...
          <form className="border-t border-border/60 bg-background pt-4" onSubmit={submitMessage}>
            <label className="sr-only" htmlFor="character-dialogue-message">{translateUi("对角色说的话")}</label>
            <textarea
              ...
              placeholder={translateUi("想对“{{value0}}”说什么？", { value0: props.characterName })}
            ...
            <div className="mt-2 flex items-center justify-between gap-3">
              ...
              <AiButton type="submit" size="sm" disabled={!props.message.trim() || props.isSending}>
                <Send className="mr-1.5 h-3.5 w-3.5" />{props.isSending ? translateUi("等待回应...") : translateUi("发送")}
              </AiButton>
            </div>
          </form>
        </>
      ) : null}
      {props.error ? <div className="border-t border-destructive/20 bg-destructive/5 px-4 py-3 text-sm text-destructive">{props.error instanceof Error ? props.error.message : translateUi("暂时无法继续这段谈话，请稍后重试。")}</div> : null}
    </section>
  );
}
  ...
  return (
    <aside className="border-t border-primary/20 bg-primary/[0.045] px-1 py-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <div className="text-sm font-semibold">{translateUi("这段谈话留下的创作倾向")}</div>
      ...
      {canDecide ? <div className="mt-3 flex flex-wrap gap-2"><AiButton size="sm" onClick={props.onActivate} disabled={props.isActivating}>{props.isActivating ? translateUi("带入中...") : translateUi("带入后续创作")}</AiButton><Button size="sm" variant="ghost" onClick={props.onDismiss} disabled={props.isDismissing}>{props.isDismissing ? translateUi("处理中...") : translateUi("本次不带入")}</Button></div> : null}
      {!canDecide && canDismiss ? <Button className="mt-3" size="sm" variant="ghost" onClick={props.onDismiss} disabled={props.isDismissing}>{props.isDismissing ? translateUi("处理中...") : translateUi("本次不带入")}</Button> : null}
    </aside>
  );
}
