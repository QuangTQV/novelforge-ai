...
export default function CharacterIntelligenceTab(props: CharacterIntelligenceTabProps) {
  ...
  if (mindQuery.isLoading) return <section className="rounded-3xl border border-dashed p-6 text-sm text-muted-foreground">{translateUi("正在整理角色的谈话场景...")}</section>;
  ...
  return (
    <CharacterConversationWorkbench
      ...
      headerActions={<AiButton variant="outline" size="sm" onClick={() => refreshMutation.mutate()} disabled={refreshMutation.isPending}><RefreshCw className="mr-1.5 h-3.5 w-3.5" />{refreshMutation.isPending ? translateUi("整理中...") : translateUi("更新场景分析")}</AiButton>}
      sidePanel={<><CharacterMindSceneAnalysis characterName={selectedCharacter.name} mind={mind} />{refreshMutation.error ? <div className="mt-3 text-sm text-destructive">{refreshMutation.error instanceof Error ? refreshMutation.error.message : translateUi("场景分析暂时无法更新，请稍后重试。")}</div> : null}</>}
    />
  );
}

function EmptyMindState(props: { characterName: string; isRefreshing: boolean; error: unknown; onRefresh: () => void }) {
  return <section className="rounded-3xl border border-dashed bg-muted/10 p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-sm font-semibold"><Brain className="h-4 w-4" />{translateUi("准备谈话场景")}</div><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{translateUi("AI 会结合角色档案、关系、已发生剧情和当前处境，整理这次谈话需要理解的角色视角。")}</p></div><AiButton onClick={props.onRefresh} disabled={props.isRefreshing}>{props.isRefreshing ? translateUi("整理中...") : translateUi("让 AI 整理谈话场景")}</AiButton></div>{props.error ? <div className="mt-3 text-sm text-destructive">{props.error instanceof Error ? props.error.message : translateUi("场景整理暂时无法完成，请稍后重试。")}</div> : null}</section>;
}
