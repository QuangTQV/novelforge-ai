  ...
  return (
    <div className="space-y-4">
      <section className="rounded-xl border border-border/70 bg-muted/10 p-4">
        <div className="mb-3">
          <div className="text-sm font-medium">{translateUi("基础档案")}</div>
          <div className="mt-1 text-xs leading-5 text-muted-foreground">

            {translateUi("这里维护角色进入章节生成时最稳定的资料。当前状态和当前目标会影响后续章节行动判断。")}
          </div>
        </div>
        <div className="grid gap-2 md:grid-cols-2">
          <Input
            placeholder={translateUi("角色名称")}
          ...
          >
            <option value="unknown">{translateUi("性别：未知")}</option>
            <option value="male">{translateUi("性别：男")}</option>
            <option value="female">{translateUi("性别：女")}</option>
            <option value="other">{translateUi("性别：其他")}</option>
          </SelectControl>
        </div>
      </section>

      <section className="rounded-xl border border-border/70 bg-background p-4">
        <div className="mb-3 text-sm font-medium">{translateUi("当前处境")}</div>
        <div className="grid gap-2 md:grid-cols-2">
          <Input
            placeholder={translateUi("当前状态（例如：重伤闭关）")}
            value={characterForm.currentState}
            onChange={(event) => onCharacterFormChange("currentState", event.target.value)}
          />
          <Input
            placeholder={translateUi("当前目标（例如：三个月内突破）")}
      ...
      <section className="rounded-xl border border-border/70 bg-background p-4">
        <div className="mb-3 text-sm font-medium">{translateUi("外显字段快速编辑")}</div>
        <div className="grid gap-2 md:grid-cols-2">
          {VISIBLE_PROFILE_FIELDS.map((field) => (
            <textarea
              key={field.key}
              className="min-h-[72px] w-full rounded-md border bg-background p-2 text-sm"
              placeholder={translateUi("{{value0}}：{{value1}}", { value0: field.label, value1: field.placeholder })}
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-border/70 bg-muted/10 p-3">
        <Button size="sm" onClick={onSaveCharacter} disabled={isSavingCharacter}>
          {isSavingCharacter ? translateUi("保存中...") : translateUi("保存角色资产")}
        </Button>
        ...
        <AiButton size="sm" variant="outline" onClick={onWorldCheck} disabled={isCheckingWorld}>
          {isCheckingWorld ? translateUi("检查中...") : translateUi("检查世界一致性")}
        </AiButton>
      </div>
    </div>
  );
