...
const MODE_OPTIONS: Array<{
  ...
  { value: "all", label: translateUi("全部关系"), icon: Network },
  { value: "current", label: translateUi("当前角色"), icon: RadioTower },
  { value: "tension", label: translateUi("高张力"), icon: AlertTriangle },
  { value: "dynamic", label: translateUi("动态阶段"), icon: GitBranch },
];
...
export default function CharacterRelationshipGraphPanel(props: CharacterRelationshipGraphPanelProps) {
  ...
  return (
    <FullscreenView
      title={translateUi("角色关系网")}
      description={translateUi("以图谱方式观察角色之间的压力、合作、秘密和下一转折点。")}
      meta={(
        <>
          <Badge variant="outline">{model.nodes.length}  {translateUi("个角色")}</Badge>
          <Badge variant="outline">{model.totalEdgeCount}  {translateUi("条关系")}</Badge>
          {model.dynamicEdgeCount > 0 ? <Badge variant="secondary">{model.dynamicEdgeCount}  {translateUi("条动态阶段")}</Badge> : null}
        </>
      )}
      ...
      toggleLabel={translateUi("全屏查看")}
      exitLabel={translateUi("退出全屏")}
            ...
            >
              ...
              <Panel position="top-left" className="rounded-xl border border-border/70 bg-background/90 px-3 py-2 text-xs text-muted-foreground shadow-sm">
                <div className="flex flex-wrap gap-3">
                  <span className="inline-flex items-center gap-1.5"><span className="h-0.5 w-6 bg-slate-500" />{translateUi("普通关系")}</span>
                  <span className="inline-flex items-center gap-1.5"><span className="h-0.5 w-6 bg-orange-500" />{translateUi("高张力")}</span>
                  <span className="inline-flex items-center gap-1.5"><span className="h-0.5 w-6 border-t-2 border-dashed border-sky-600" />{translateUi("动态阶段")}</span>
                </div>
              </Panel>
              {mode === "current" ? (
                <Panel position="bottom-left" className="rounded-xl border border-border/70 bg-background/90 px-3 py-2 text-xs text-muted-foreground shadow-sm">
                  ...
                  {translateUi("当前角色在左侧，直接关系向右展开；点击连线查看关系细节。")}
                </Panel>
              ) : null}
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 line-clamp-2 min-h-[36px] text-xs leading-[18px] text-muted-foreground">
        {character.currentGoal || character.storyFunction || character.relationToProtagonist || translateUi("待补全角色目标")}
      </div>
      <div className="absolute inset-x-3.5 bottom-2.5 flex flex-wrap gap-1.5">
        <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-medium", tone.badge)}>
          {isProtagonist ? translateUi("主角核心") : getCastRoleLabel(character.castRole)}
        </span>
        <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">
          {graphNode.relationCount}  {translateUi("关系")}
        </span>
        {graphNode.dynamicCount > 0 ? (
          <span className="rounded-full bg-sky-50 px-2 py-0.5 text-[10px] text-sky-700">
            {graphNode.dynamicCount}  {translateUi("动态")}
          </span>
        ) : null}
      </div>
    </div>
    ...
    <div className="space-y-4">
      <div className={cn(
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isProtagonist ? <Badge className="border-emerald-200 bg-emerald-50 text-emerald-800" variant="outline">{translateUi("叙事核心")}</Badge> : null}
        </div>
        <div className="mt-2 text-sm leading-6 text-muted-foreground">{character.role || translateUi("未定义身份")}</div>
      </div>
      <DetailBlock title={translateUi("当前目标")} value={character.currentGoal} />
      <DetailBlock title={translateUi("当前状态")} value={character.currentState} />
      <DetailBlock title={translateUi("故事作用")} value={character.storyFunction} />
      <DetailBlock title={translateUi("与主角关系")} value={character.relationToProtagonist} />
      <div className="grid grid-cols-2 gap-2 text-xs">
        <MiniStat icon={<UsersRound className="h-3.5 w-3.5" />} label={translateUi("关系数")} value={String(props.node.relationCount)} />
        <MiniStat icon={<GitBranch className="h-3.5 w-3.5" />} label={translateUi("动态阶段")} value={String(props.node.dynamicCount)} />
      </div>
    </div>
  );
}
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-border/70 bg-muted/10 p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {edge.isDynamic ? <Badge variant="secondary">{translateUi("动态阶段")}</Badge> : null}
          {edge.isHighTension ? <Badge variant="outline">{translateUi("高张力")}</Badge> : null}
        </div>
        <div className="mt-2 text-sm text-muted-foreground">
          {getRelationNames(edge)}
        </div>
      </div>
      <DetailBlock title={translateUi("表层关系")} value={relation?.surfaceRelation} />
      <DetailBlock title={translateUi("当前阶段")} value={currentStage?.stageSummary ?? currentStage?.stageLabel} />
      <DetailBlock title={translateUi("隐藏张力")} value={relation?.hiddenTension} />
      <DetailBlock title={translateUi("冲突来源")} value={relation?.conflictSource} />
      <DetailBlock title={translateUi("秘密不对称")} value={relation?.secretAsymmetry} />
      <DetailBlock title={translateUi("下一转折点")} value={currentStage?.nextTurnPoint ?? relation?.nextTurnPoint} />
      <div className="grid grid-cols-2 gap-2 text-xs">
        <MiniStat icon={<Sparkles className="h-3.5 w-3.5" />} label={translateUi("阶段数")} value={String(edge.dynamicStages.length)} />
        <MiniStat icon={<AlertTriangle className="h-3.5 w-3.5" />} label={translateUi("风险")} value={edge.isHighTension ? translateUi("高") : translateUi("普通")} />
      </div>
      {currentStage?.chapterOrder ? (
        <div className="rounded-xl border border-border/70 bg-background p-3 text-xs text-muted-foreground">
          ...
          {translateUi("最近推进：第")} {currentStage.chapterOrder}  {translateUi("章")}
        </div>
      ) : null}
    </div>
  );
  ...
  return (
    <div className="flex h-full min-h-[520px] items-center justify-center p-6">
      <div className="max-w-md rounded-2xl border border-dashed bg-background/85 p-6 text-center">
        ...
        <div className="mt-4 text-sm font-medium">{translateUi("关系网还没有可绘制内容")}</div>
        <div className="mt-2 text-sm leading-6 text-muted-foreground">

          {translateUi("先应用一套角色阵容，或完成几章后同步角色动态；系统会把表层关系、隐藏张力和动态阶段汇总到这里。")}
        </div>
      </div>
    </div>
  );
