    ...
    <div className="space-y-4">
      <section className="rounded-xl border border-border/70 bg-muted/10 p-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="text-sm font-medium">{translateUi("关键资源")}</div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {isBackfillingCharacterResources ? translateUi("回填中...") : translateUi("回填最近章节")}
            </Button>
            <Badge variant="outline">{resourceDisplayMode.label}</Badge>
            {pendingCharacterResourceCount > 0 ? (
              <Badge variant="secondary">{pendingCharacterResourceCount}  {translateUi("条资源变更待确认")}</Badge>
            ) : null}
          </div>
        </div>
      </section>
      ...
      {displayedResources.length > 0 ? (
          ...
          {displayedResources.map((resource) => (
            <div key={resource.id} className="rounded-lg border border-border/70 bg-background p-3">
              ...
              <div className="mt-2 grid gap-1 text-xs text-muted-foreground sm:grid-cols-2">
                <div>{translateUi("持有者：")}{resource.holderCharacterName || selectedCharacter.name}</div>
                <div>{translateUi("读者知情：")}{resource.readerKnows ? translateUi("知情") : translateUi("未公开")}</div>
                {resource.expectedUseEndChapterOrder ? (
                  <div>{translateUi("使用窗口：第")}{resource.expectedUseStartChapterOrder ?? "?"}{translateUi("章至第")}{resource.expectedUseEndChapterOrder}{translateUi("章")}</div>
                ) : null}
                {resource.constraints.length > 0 ? (
                  <div>{translateUi("限制：")}{resource.constraints.slice(0, 2).join(" / ")}</div>
                ) : null}
              </div>
            </div>
          ))}
        ...
        <div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
          ...
          {translateUi("关键道具、线索、身份凭证或底牌会在章节写作后沉淀到这里；临时角色只保留会影响后续章节的资源。")}
        </div>
      )}
    </div>
  );
