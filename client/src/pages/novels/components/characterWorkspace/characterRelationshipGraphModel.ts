...
export function buildRelationshipGraphModel(input: {
  ...
  for (const relation of input.staticRelations) {
    ...
    edgesByPair.set(pairKey, {
      ...
      sourceName: relation.sourceCharacterName || characterById.get(relation.sourceCharacterId)?.name || translateUi("未知角色"),
      targetName: relation.targetCharacterName || characterById.get(relation.targetCharacterId)?.name || translateUi("未知角色"),
  ...
  for (const stage of input.dynamicRelations) {
    ...
    if (existing) {
      ...
      edgesByPair.set(pairKey, {
        ...
        sourceName: stage.sourceCharacterName || characterById.get(stage.sourceCharacterId)?.name || translateUi("未知角色"),
        targetName: stage.targetCharacterName || characterById.get(stage.targetCharacterId)?.name || translateUi("未知角色"),
