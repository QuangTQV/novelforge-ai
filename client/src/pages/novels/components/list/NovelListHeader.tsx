    ...
    <section className="space-y-4">
        ...
        <div className="min-w-0 space-y-3">
          <div>
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-3xl font-semibold tracking-normal">{props.view === "shelf" ? translateUi("我的书架") : translateUi("小说工作台")}</h1>
              <div className="inline-flex rounded-md bg-muted/50 p-1" role="tablist" aria-label={translateUi("小说列表视图")}>
                <Button type="button" size="sm" variant={props.view === "shelf" ? "default" : "ghost"} onClick={() => props.onViewChange("shelf")} role="tab" aria-selected={props.view === "shelf"}>
                  <Library className="mr-1.5 h-4 w-4" aria-hidden="true" />{translateUi("书架")}
                </Button>
                <Button type="button" size="sm" variant={props.view === "workbench" ? "default" : "ghost"} onClick={() => props.onViewChange("workbench")} role="tab" aria-selected={props.view === "workbench"}>
                  <LayoutDashboard className="mr-1.5 h-4 w-4" aria-hidden="true" />{translateUi("工作台")}
                </Button>
              </div>
            </div>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              {props.view === "shelf" ? translateUi("浏览你的作品，打开封面，继续阅读或继续创作。") : translateUi("管理正在推进的小说项目，快速判断哪些可以继续写、哪些需要先处理状态。")}
            </p>
          </div>
        </div>
      ...
      {props.view === "workbench" ? (
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border-y border-border/60 py-3 text-sm">
          <HeaderMetric label={translateUi("当前")} value={translateUi("第 {{value0}} / {{value1}} 页", { value0: props.page, value1: props.totalPages })} />
          <HeaderMetric label={translateUi("总数")} value={translateUi("{{value0}} 本", { value0: props.totalNovels })} />
          ...
          {props.recoveryCandidateCount > 0 ? (
            <Button type="button" size="sm" variant="ghost" className="h-8 px-2" onClick={props.onOpenRecovery}>
              ...
              {translateUi("待恢复")} {props.recoveryCandidateCount}
            </Button>
          ) : null}
        </div>
      ) : null}
