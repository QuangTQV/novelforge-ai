    ...
    <>
      <div id="novel-world-source" className="rounded-lg border border-border/70 bg-muted/20 p-4">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="text-sm font-medium text-foreground">{translateUi("选择本书世界来源")}</div>
            <div className="mt-1 text-sm leading-6 text-muted-foreground">

              {translateUi("先决定这本小说的世界从哪里来，再展开对应操作。新建世界会同时保存到世界库，并与本书关联。")}
            </div>
          </div>
          <Badge variant="outline">{translateUi("本书副本")}</Badge>
        </div>
        <div className="mt-3 grid gap-3 lg:grid-cols-3">
          <WorldSetupChoice
            icon={Sparkles}
            title={translateUi("根据本书生成")}
            description={translateUi("适合还没有明确世界设定时，让系统根据标题、简介、卖点和类型生成本书世界。")}
            selected={worldSetupMode === "generate"}
            onSelect={() => setWorldSetupMode("generate")}
          />
          <WorldSetupChoice
            icon={Download}
            title={translateUi("从样本库导入")}
            description={translateUi("适合已有可复用世界样本时，复制一份作为本书世界，再决定是否手动同步。")}
            selected={worldSetupMode === "import"}
            onSelect={() => setWorldSetupMode("import")}
          />
          <WorldSetupChoice
            icon={BookOpen}
            title={translateUi("自定义空白手册")}
            description={translateUi("适合你有明确想法时，先创建本书世界骨架，再逐步补齐规则、势力和地点。")}
      ...
      {worldSetupMode === "import" ? (
        <div className="rounded-lg border border-border/70 bg-background p-4">
          <div className="text-sm font-medium text-foreground">{translateUi("从样本库导入")}</div>
          <div className="mt-1 text-sm leading-6 text-muted-foreground">

            {translateUi("导入会复制外部世界手册。本书生成时使用这份副本，外部世界库不会被自动改动。")}
          </div>
          <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
            ...
            >
              <option value="">{translateUi("选择一个世界样本")}</option>
            ...
            >
              ...
              {props.isImporting ? translateUi("导入中...") : translateUi("导入为本书世界")}
            </Button>
          </div>
          <label className="mt-3 flex items-start gap-3 text-sm text-muted-foreground">
            ...
            />
            <span>{translateUi("导入后保留同步入口。系统只提示差异，不会自动覆盖本书世界或世界库样本。")}</span>
          </label>
        </div>
      ) : null}
      ...
      {worldSetupMode === "generate" ? (
        <div className="rounded-lg border border-border/70 bg-background p-4">
          <div className="text-sm font-medium text-foreground">{translateUi("根据本书生成世界")}</div>
          <div className="mt-1 text-sm leading-6 text-muted-foreground">

            {translateUi("系统会根据标题、简介、卖点、读者承诺和类型信息生成一套本书世界，同时保存到世界库，方便后续复用和维护。")}
          </div>
          <div className="mt-3 flex justify-end">
            ...
            >
              ...
              {props.isGenerating ? translateUi("生成中...") : translateUi("生成本书世界")}
            </Button>
          </div>
        </div>
      ) : null}
      ...
      {worldSetupMode === "manual" ? (
        <div className="rounded-lg border border-border/70 bg-background p-4">
          <div className="text-sm font-medium text-foreground">{translateUi("自定义本书世界")}</div>
          <div className="mt-1 text-sm leading-6 text-muted-foreground">

            {translateUi("先创建一份空白世界手册，并同步保存到世界库；再到世界工作台补齐核心规则、主要势力、故事舞台和关键张力。")}
          </div>
          <div className="mt-3 grid gap-3 lg:grid-cols-2">
            <label className="space-y-1 text-sm">
              <span className="font-medium text-foreground">{translateUi("世界名称")}</span>
              <input
                className="w-full rounded-md border bg-background p-2 text-sm"
                value={manualWorldTitle}
                maxLength={80}
                placeholder={translateUi("例如：紫霞界")}
                onChange={(event) => setManualWorldTitle(event.target.value)}
              />
            </label>
            <label className="space-y-1 text-sm">
              <span className="font-medium text-foreground">{translateUi("一句话概要")}</span>
              <input
                className="w-full rounded-md border bg-background p-2 text-sm"
                value={manualWorldSummary}
                maxLength={300}
                placeholder={translateUi("例如：星核枯竭的边境帝国，魔法与权力都要付出代价。")}
          ...
          <div className="mt-3 flex justify-end">
            ...
            >
              ...
              {props.isCreatingManual ? translateUi("创建中...") : translateUi("自定义本书世界")}
            </Button>
          </div>
        </div>
      ) : null}
