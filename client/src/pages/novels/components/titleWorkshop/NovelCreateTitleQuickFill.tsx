...
export default function NovelCreateTitleQuickFill({
  ...
  const handleApplyTitle = (title: string, source: "generated" | "library") => {
    onApplyTitle(title);
    setOpen(false);
    toast.success(source === "generated" ? translateUi("标题候选已写入创建表单。") : translateUi("标题库标题已写入创建表单。"));
  };
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[85vh] max-w-5xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{translateUi("标题快速选填")}</DialogTitle>
            <DialogDescription>

              {translateUi("不做绑定关系，只是帮你更快把标题写进创建表单。可以直接生成候选，也可以从标题库挑一个回填。")}
            </DialogDescription>
          </DialogHeader>
          ...
          >
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="generate">{translateUi("快速生成")}</TabsTrigger>
              <TabsTrigger value="library">{translateUi("标题库选择")}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="generate" className="space-y-4">
              <div className="space-y-4">
                <div className="text-xs leading-6 text-muted-foreground">
                  ...
                  {translateUi("会优先读取当前创建页里已经填写的简介、题材、文风、节奏和叙事视角。你也可以在下面临时补充一句简报，不用先回到表单里填写。")}
                </div>
                <div>
                  <LLMSelector />
                </div>
                ...
                <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_280px]">
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      placeholder={translateUi("例如：末世废土里，一个被流放的维修师意外掌握古代机甲核心，想要标题更有硬核设定感和命运感。")}
                    />
                    <div className="text-xs leading-6 text-muted-foreground">

                      {translateUi("这里只影响这一次生成，不会自动回写到小说创建表单。")}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder={translateUi("可选，填了会按参考改编式生成")}
                      />
                    </div>
                    <div className="text-xs leading-6 text-muted-foreground">
                      {referenceTitle.trim()
                        ? translateUi("当前会参考你输入的标题节奏和命名结构，再结合这本小说的信息重新产出候选。")
                        : translateUi("留空时会按简报直接生成。如果你心里已经有一个风格方向，可以在这里填参考标题。")}
                    </div>
                  </div>
                </div>

                <div className="border-l border-border/60 pl-3">
                  <div className="text-xs font-medium text-foreground">{translateUi("当前已自动读取的创建页信息")}</div>
                  <div className="mt-2 whitespace-pre-wrap text-xs leading-6 text-muted-foreground">
                    {autoBrief || translateUi("创建页里暂时还没有足够的信息。你可以直接在上面的“补充标题简报”里写一句题材、卖点或冲突再生成。")}
                  </div>
                </div>

                <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                  <label className="space-y-2 text-sm">
                    <span className="font-medium text-foreground">{translateUi("生成数量")}</span>
                    <Input
                      type="number"
                      min={3}
                      max={24}
                ...
                {!hasGenerationContext ? (
                  <div className="mt-3 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-800">
                    ...
                    {translateUi("至少先补一句标题简报，或填写一个参考标题；如果创建页里已经有简介、类型或文风，也会自动参与生成。")}
                  </div>
                ) : null}
              </div>
              ...
              <TitleSuggestionList
                ...
                primaryActionLabel={translateUi("填入标题")}
                ...
                emptyMessage={translateUi("可以直接在上面的补充标题简报里写一句题材或卖点，再点一次生成，结果会直接作为创建页的标题候选。")}
              />
            </TabsContent>
            ...
            <TabsContent value="library" className="space-y-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="space-y-1">
                  <div className="text-sm font-medium text-foreground">{translateUi("从标题库快速选用")}</div>
                  <div className="text-xs leading-6 text-muted-foreground">

                    {translateUi("默认按点击率排序")}
                    {basicForm.genreId ? translateUi("，并按当前题材基底过滤") : ""}
                ...
                <Input
                  ...
                  placeholder={translateUi("搜索标题关键词")}
              ...
              {libraryQuery.isLoading ? (
                ...
                <div className="py-8 text-center text-sm text-muted-foreground">
                  ...
                  {translateUi("当前条件下还没有可用标题。可以切到“快速生成”先产出一批候选。")}
                </div>
              ) : (
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              {typeof entry.clickRate === "number" ? (
                                <Badge className={getClickRateBadgeClass(entry.clickRate)}>
                                  ...
                                  {translateUi("预估")} {entry.clickRate}
                                </Badge>
                              ) : null}
