...
export function useStorylineVersionControl({
  ...
  const createDraftVersionMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      setStorylineMessage(response.message ?? translateUi("主线草稿版本已创建。"));
      await invalidateVersionList();
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : translateUi("创建主线草稿版本失败。");
  ...
  const activateVersionMutation = useMutation({
    mutationFn: () => {
      if (!selectedVersionId) {
        throw new Error(translateUi("请先选择一个主线版本。"));
      }
      return activateStorylineVersion(novelId, selectedVersionId);
    },
    onSuccess: async (response) => {
      setStorylineMessage(response.message ?? translateUi("已设为生效主线。"));
      await invalidateVersionList();
      await invalidateNovelDetail();
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : translateUi("设置生效版失败。");
      setStorylineMessage(message);
    },
  });
  ...
  const freezeVersionMutation = useMutation({
    ...
    onSuccess: async (response) => {
      setStorylineMessage(response.message ?? translateUi("主线版本已冻结。"));
      await invalidateVersionList();
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : translateUi("冻结主线版本失败。");
      setStorylineMessage(message);
    },
  });
  ...
  const diffMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setStorylineMessage(response.message ?? translateUi("主线版本差异已更新。"));
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : translateUi("加载版本差异失败。");
      setStorylineMessage(message);
    },
  });
  ...
  const analyzeDraftImpactMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setStorylineMessage(response.message ?? translateUi("草稿影响分析完成。"));
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : translateUi("草稿影响分析失败。");
      setStorylineMessage(message);
    },
  });
  ...
  const analyzeVersionImpactMutation = useMutation({
    ...
    onSuccess: (response) => {
      setImpactResult(response.data ?? null);
      setStorylineMessage(response.message ?? translateUi("版本影响分析完成。"));
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : translateUi("版本影响分析失败。");
      setStorylineMessage(message);
    },
  });
  ...
  const loadSelectedVersionToDraft = () => {
    ...
    setStorylineMessage(translateUi("已加载 V{{v0}} 到当前草稿。", { v0: selectedVersion.version }));
  };
