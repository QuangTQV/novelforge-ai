        ...
        <div className="flex min-w-0 items-start justify-between gap-3">
          ...
          <Dialog open={isToolsOpen} onOpenChange={setIsToolsOpen}>
            <DialogTrigger asChild>
              <Button type="button" size="icon" variant="outline" className="shrink-0" aria-label={translateUi("打开创作工具")}>
                <MoreHorizontal className="h-4 w-4" />
              </Button>
            </DialogTrigger>
            <DialogContent className="max-h-[88vh] w-[calc(100vw-1.5rem)] overflow-y-auto rounded-2xl">
              <DialogHeader>
                <DialogTitle>{translateUi("创作工具")}</DialogTitle>
                <DialogDescription>{translateUi("查看任务进度，导出当前步骤或整本书内容。")}</DialogDescription>
              </DialogHeader>
              <div className="space-y-3 text-sm">
                <div className="grid grid-cols-3 gap-2">
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-3">
                    <div className="text-xs text-muted-foreground">{translateUi("章节")}</div>
                    <div className="mt-1 font-semibold">{generatedChapters}/{Math.max(totalChapters, 1)}</div>
                  </div>
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-3">
                    <div className="text-xs text-muted-foreground">{translateUi("待修复")}</div>
                    <div className="mt-1 font-semibold">{pendingRepairs}</div>
                  </div>
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-3">
                    <div className="text-xs text-muted-foreground">{translateUi("任务")}</div>
                    <div className="mt-1 truncate font-semibold">{taskAttentionLabel ?? translateUi("无")}</div>
                  </div>
                </div>
                ...
                {taskDrawer ? (
                  ...
                  >
                    <span>{translateUi("查看任务进度")}</span>
                    {taskAttentionLabel ? <Badge variant="secondary">{taskAttentionLabel}</Badge> : null}
                  </Button>
                ) : null}

                <div className="rounded-xl border border-border/70 p-3">
                  <div className="text-sm font-medium">{translateUi("导出当前步骤")}</div>
                ...
                <div className="rounded-xl border border-border/70 p-3">
                  <div className="text-sm font-medium">{translateUi("导出整本书")}</div>
