    ...
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            <div className="font-medium text-foreground">{translateUi("AI 已准备的创作资源")}</div>
            <div className="mt-1 text-xs leading-5 text-muted-foreground">

              {translateUi("只读查看故事承诺、世界、角色和分卷成果；AI 会继续用这些资源保持后续章节一致。")}
            </div>
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <div className="hidden flex-wrap gap-2 text-xs sm:flex">
            <Badge variant="outline">{materials.characterCount}  {translateUi("位角色")}</Badge>
            <Badge variant="outline">{materials.volumeCount}  {translateUi("卷规划")}</Badge>
          </div>
      ...
      <div className="grid gap-3 border-t border-border/60 p-4 md:grid-cols-2 xl:grid-cols-4">
        <ResourceCard icon={Sparkles} title={translateUi("整书方向")} meta={translateUi("故事承诺")}>
          ...
          {materials.story.first30ChapterPromise ? (
            <p className="mt-2">{translateUi("前期兑现：")}{materials.story.first30ChapterPromise}</p>
          ) : null}
        </ResourceCard>

        <ResourceCard
          icon={Globe2}
          title={translateUi("故事世界")}
          meta={materials.world ? translateUi("准备完成") : translateUi("准备中")}
        >
          {materials.world ? (
            <>
              <p className="font-medium text-foreground">{materials.world.name}</p>
              <p className="mt-1">{materials.world.summary || translateUi("核心世界规则已纳入章节生产。")}</p>
            </>
          ) : (
            <p>{translateUi("世界规则完成后会在这里显示摘要。")}</p>
          )}
        </ResourceCard>

        <ResourceCard icon={Users} title={translateUi("角色阵容")} meta={translateUi("{{value0}} 位", { value0: materials.characterCount })}>
            ...
            <div className="space-y-2">
              {displayedCharacters.map((character) => (
                <div key={character.id}>
                  ...
                  {character.currentGoal ? <div className="text-xs leading-5">{translateUi("当前目标：")}{character.currentGoal}</div> : null}
                </div>
              ))}
              {materials.characterCount > displayedCharacters.length ? (
                <div className="text-xs">{translateUi("另有")} {materials.characterCount - displayedCharacters.length}  {translateUi("位角色已纳入生产。")}</div>
              ) : null}
            </div>
          ) : (
            <p>{translateUi("角色阵容准备完成后会在这里显示。")}</p>
          )}
        </ResourceCard>

        <ResourceCard icon={BookMarked} title={translateUi("分卷路线")} meta={translateUi("{{value0}} 卷", { value0: materials.volumeCount })}>
          {materials.volumes.length > 0 ? (
            <div className="space-y-2">
              {displayedVolumes.map((volume) => (
                <div key={volume.id}>
                  <span className="font-medium text-foreground">{translateUi("第")} {volume.order}  {translateUi("卷 ·")} {volume.title}</span>
                  <div className="text-xs leading-5">
                    {volume.chapterCount}  {translateUi("章")}{volume.mainPromise ? ` · ${volume.mainPromise}` : ""}
                  </div>
                </div>
              ))}
              {materials.volumeCount > displayedVolumes.length ? <div className="text-xs">{translateUi("其余分卷已纳入全书生产。")}</div> : null}
            </div>
          ) : (
            <p>{translateUi("卷战略与章节路线完成后会在这里显示。")}</p>
          )}
        </ResourceCard>
      </div>
    </details>
