...
const REFERENCE_GROUP_LABELS: Record<PromptTemplateReferenceItem["group"], string> = {
  required_context: translateUi("必需上下文"),
  optional_context: translateUi("可选上下文"),
  input: translateUi("运行变量"),
  slot: translateUi("槽位"),
};
    ...
    <div className="rounded-md border border-[#cbdad6] bg-white shadow-[0_18px_40px_rgba(20,54,48,0.16)]">
      <div className="border-b border-[#dce8e4] p-2">
        <Input
          ...
          placeholder={translateUi("搜索上下文、变量或槽位")}
          className="h-8 border-[#cbdad6]"
        />
      </div>
      <div className="max-h-80 overflow-auto p-2">
        {grouped.length === 0 ? (
          <div className="px-2 py-3 text-sm text-muted-foreground">{translateUi("没有可插入的引用。")}</div>
        ) : grouped.map((section) => (
          <div key={section.group} className="mb-2 last:mb-0">
            <div className="px-2 pb-1 text-[11px] font-semibold text-[#52606d]">
              {translateUi(REFERENCE_GROUP_LABELS[section.group])}
            </div>
...
function PromptTokenElement(props: {
  ...
  const title = [
    ...
    keyText,
    element.description,
    element.required ? translateUi("必需上下文") : "",
    element.hasPreviewBlock === false ? translateUi("当前预览未装配内容") : "",
  ].filter(Boolean).join("\n");
    ...
    <div className="relative rounded-md border border-[#d7e4e0] bg-white">
      <div className="flex items-center justify-between gap-3 border-b border-[#e1ebe8] px-3 py-2">
        <div>
          ...
          <div className="text-[11px] text-muted-foreground">{translateUi("源码调试视图会显示原始模板 token")}</div>
        </div>
        <Button
          type="button"
          variant="outline"
  ...
  return (
    <div ref={rootRef} className="relative rounded-md border border-[#d7e4e0] bg-white">
      <div className="flex items-center justify-between gap-3 border-b border-[#e1ebe8] px-3 py-2">
        <div>
          <div className="text-sm font-semibold text-[#25443f]">{props.label}</div>
          <div className="text-[11px] text-muted-foreground">{translateUi("输入 @ 可插入上下文、变量或槽位标签")}</div>
        </div>
          ...
          {editor ? (
            <Plate editor={editor} onValueChange={handleValueChange}>
              <PlateContent
                ...
                placeholder={translateUi("编排提示词内容，可插入上下文标签")}
