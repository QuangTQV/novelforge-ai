  ...
  return (
    <div className={`grid gap-3 ${showProtocolFields ? "md:grid-cols-6" : "md:grid-cols-4"}`}>
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">{translateUi("服务商")}</div>
        <Select
        ...
        >
          <SelectTrigger>
            <SelectValue placeholder={translateUi("选择服务商")} />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">{translateUi("模型")}</div>
        <SearchableSelect
      ...
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">{translateUi("温度")}</div>
        <Input
      ...
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">{translateUi("最大输出长度")}</div>
        <Input
      ...
      {showProtocolFields ? (
        <div className="space-y-1">
          <div className="text-xs text-muted-foreground">{translateUi("请求协议")}</div>
          <Select
          ...
          >
            <SelectTrigger>
              <SelectValue placeholder={translateUi("自动选择")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="auto">{translateUi("自动选择")}</SelectItem>
              <SelectItem value="openai_compatible">{translateUi("OpenAI 兼容")}</SelectItem>
      ...
      {showProtocolFields ? (
        <div className="space-y-1">
          <div className="text-xs text-muted-foreground">{translateUi("结构化格式")}</div>
          <Select
            ...
            <SelectContent>
              {getStructuredResponseFormatOptions(draft.requestProtocol).map((format) => (
                <SelectItem key={format} value={format}>
                  {format === "auto" ? translateUi("自动选择") : format}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
