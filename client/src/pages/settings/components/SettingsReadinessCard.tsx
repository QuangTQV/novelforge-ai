...
function getReadinessBadge(state: SettingsReadinessItem["state"]) {
  switch (state) {
    case "ready":
      return translateUi("可用");
    case "checking":
      return translateUi("检查中");
    case "optional":
      return translateUi("可选增强");
    case "warning":
      return translateUi("需要处理");
  }
}
...
export function buildSettingsReadinessItems(input: {
  ...
  return [
    {
      key: "model",
      title: translateUi("正文模型"),
      state: runnableProviders.length > 0 ? "ready" : "warning",
      description: runnableProviders.length > 0
        ? translateUi("已可使用 {{value0}} 进行正文与规划生成。", { value0: runnableProviders[0].name })
        : translateUi("先配置一个可用模型，就可以开始开书和生成章节。"),
    },
    {
      key: "routes",
      title: translateUi("模型路由"),
      state: isModelRoutesChecking ? "checking" : hasRoutes && failedRouteCount === 0 ? "ready" : "warning",
      description: isModelRoutesChecking
        ? translateUi("正在检查开书、拆章、正文生成和审核任务的模型兼容性。")
        : hasRoutes && failedRouteCount === 0
          ? translateUi("创作任务已有可用路由，后续流程会按任务选择模型。")
          : translateUi("部分创作任务还需要补齐或修复模型路由。"),
    },
    {
      key: "rag",
      title: translateUi("知识库增强"),
      ...
      description: ragSettings?.enabled && currentRagProvider?.isConfigured && currentRagProvider?.isActive
        ? translateUi("知识库检索已启用，可帮助长篇写作保持资料和设定连续。")
        : translateUi("不配置也可以开始创作；启用后会增强设定、资料和上下文召回。"),
    },
    {
      key: "style",
      title: translateUi("写法引擎"),
      state: !isStyleSettingsLoaded ? "checking" : styleReady ? "ready" : "warning",
      description: styleReady
        ? translateUi("写法提取等待时间在可用范围内，可用于学习样本文风。")
        : translateUi("请确认写法提取等待时间在可用范围内。"),
    },
  ];
}
...
export default function SettingsReadinessCard(props: {
  ...
  const primaryAction = !hasModel
    ? { label: translateUi("配置正文模型"), to: "/settings/models" }
    : !hasHealthyRoutes
      ? { label: translateUi("检查模型路由"), to: "/settings/models" }
      : { label: translateUi("开始创建小说"), to: "/novels/create" };

  return (
    <Card className="min-w-0 overflow-hidden border-primary/20 bg-primary/5">
      <CardHeader className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 space-y-1">
          <CardTitle>{translateUi("创作可用性检查")}</CardTitle>
          <CardDescription className={AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}>

            {translateUi("先确认开始写小说必需的模型和路由是否可用；知识库属于增强项，可以稍后再补。")}
          </CardDescription>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className={`text-sm ${canStart ? "text-emerald-700" : "text-muted-foreground"} ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
          {canStart
            ? translateUi("基础创作链路已经可用，可以开始创建或继续推进小说。")
            : translateUi("先处理标记为“需要处理”的项目，完成后再进入自动导演或章节生产会更稳。")}
        </div>
      </CardContent>
    </Card>
  );
