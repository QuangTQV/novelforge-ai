  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid gap-3 rounded-xl bg-muted/25 p-4 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
        <div>
          <div className="text-[11px] text-muted-foreground">{translateUi("当前阶段")}</div>
          <div className="mt-1 leading-5 text-foreground/90">{currentStage}</div>
        </div>
        <div>
          <div className="text-[11px] text-muted-foreground">{translateUi("正在处理")}</div>
      ...
      <details className="group border-t border-border/35 pt-3">
        <summary className="flex cursor-pointer list-none items-center justify-between text-xs font-medium text-muted-foreground marker:hidden">
          ...
          {translateUi("运行信息")}
          <span className="group-open:hidden">{translateUi("展开")}</span>
          <span className="hidden group-open:inline">{translateUi("收起")}</span>
        </summary>
        <dl className="mt-3 grid gap-x-4 gap-y-2 text-xs sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
          {task.kind === "novel_workflow" ? (
            <>
              <div><dt className="text-muted-foreground">{translateUi("恢复目标页")}</dt><dd className="mt-0.5 text-foreground/85">{formatResumeTarget(task.resumeTarget)}</dd></div>
              <div><dt className="text-muted-foreground">{translateUi("最近健康阶段")}</dt><dd className="mt-0.5 text-foreground/85">{task.lastHealthyStage ?? translateUi("暂无")}</dd></div>
            </>
          ) : null}
