...
const POLICY_OPTIONS: Array<{ value: DirectorPolicyMode; label: string; description: string }> = [
  {
    value: "suggest_only",
    label: translateUi("只给建议"),
    description: translateUi("只分析和给出建议，不自动写入规划或正文。"),
  },
  {
    value: "run_next_step",
    label: translateUi("推进下一步"),
    description: translateUi("只执行当前最小步骤，完成后停下来让你检查。"),
  },
  {
    value: "run_until_gate",
    label: translateUi("推进到检查点"),
    description: translateUi("连续推进到下一个需要确认的节点。"),
  },
  {
    value: "auto_safe_scope",
    label: translateUi("安全范围自动推进"),
    description: translateUi("仅在系统判断风险较低的范围内继续自动处理。"),
  },
];
...
export default function TaskCenterRuntimePolicyCard({
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success(translateUi("导演推进方式已更新"));
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : translateUi("更新导演推进方式失败"));
    },
  });
  ...
  return (
    <div className="rounded-md border bg-muted/20 p-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="font-medium">{translateUi("导演推进方式")}</div>
          <div className="mt-1 text-sm leading-6 text-muted-foreground">

            {translateUi("选择系统接下来怎么推进这个导演任务。")}
          </div>
        </div>
      ...
      <div className="mt-3 space-y-2 rounded-md border bg-background/70 p-3">
        <label className="flex items-start gap-2 text-sm">
          ...
          />
          <span>
            <span className="block font-medium">{translateUi("允许执行更完整的审校")}</span>
            <span className="block text-xs leading-5 text-muted-foreground">

              {translateUi("用于章节质量检查、近期章节复盘等步骤，系统会在执行前记录策略。")}
            </span>
          </span>
        </label>
        <label className="flex items-start gap-2 text-sm">
          ...
          />
          <span>
            <span className="block font-medium">{translateUi("允许改写受保护的内容")}</span>
            <span className="block text-xs leading-5 text-muted-foreground">

              {translateUi("仅在你确认要让系统处理已编辑正文或关键设定时开启。")}
            </span>
          </span>
        </label>
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        >
          {mutation.isPending ? translateUi("保存中...") : translateUi("保存推进方式")}
        </Button>
      </div>
    </div>
  );
