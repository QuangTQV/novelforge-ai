  ...
  return (
    <div className="rounded-md border p-3 text-sm space-y-3">
      <div className="space-y-1">
        <div className="font-medium">{translateUi("素材库前置引入")}</div>
        <div className="text-xs text-muted-foreground">

          {translateUi("旧版 V2 会在正式生成前就允许从属性库选素材。这里保留这个思路，但接到当前蓝图结构里。")}
        </div>
      </div>

      <input
        className="w-full rounded-md border p-2 text-sm"
        placeholder={translateUi("搜索素材库条目，可按名称或关键词过滤")}
        value={keyword}
      ...
      <div className="space-y-2">
        {libraryQuery.isLoading ? (
          <div className="text-xs text-muted-foreground">{translateUi("正在加载素材库…")}</div>
        ) : null}
        {!libraryQuery.isLoading && libraryItems.length === 0 ? (
          <div className="text-xs text-muted-foreground">

            {translateUi("没有匹配的素材库条目。")}
          </div>
        ) : null}
            ...
            <div key={item.id} className="flex flex-col gap-3 rounded-md border p-3 md:flex-row md:items-start md:justify-between">
              <div className="space-y-1">
                ...
                <div className="text-xs text-muted-foreground whitespace-pre-wrap">
                  {item.description?.trim() || translateUi("暂无描述")}
                </div>
                <div className="text-xs text-muted-foreground">

                  {translateUi("使用次数：")}{item.usageCount}
                </div>
              </div>
              <Button
                type="button"
                ...
                onClick={() => onAdd(item)}
              >
                {added ? translateUi("已加入蓝图") : translateUi("加入蓝图")}
              </Button>
            </div>
          );
        })}
