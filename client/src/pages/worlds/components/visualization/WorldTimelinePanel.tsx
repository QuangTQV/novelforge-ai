    ...
    <article className="rounded-2xl border border-border/50 bg-background/95 p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div className="inline-flex min-w-0 items-center gap-1.5 rounded-full bg-primary/8 px-2.5 py-1 text-xs font-semibold text-primary">
          ...
          <span className="truncate">{item.year || translateUi("阶段 {{value0}}", { value0: index + 1 })}</span>
        </div>
        <span className="shrink-0 text-[10px] tabular-nums text-muted-foreground">{translateUi("事件")} {index + 1}</span>
      </div>
...
export default function WorldTimelinePanel({ items }: WorldTimelinePanelProps) {
  ...
  return (
    <FullscreenView
      title={translateUi("世界时间线 · {{value0}} 个关键阶段", { value0: items.length })}
      description={translateUi("沿时间顺序阅读关键事件，观察世界局势、势力目标与冲突如何向前推进。")}
      fullscreen={isFullscreen}
      onFullscreenChange={setIsFullscreen}
      toggleLabel={translateUi("全屏查看时间线")}
      exitLabel={translateUi("退出时间线全屏")}
      ...
      {items.length === 0 ? (
        ...
        <>
          <div
            ...
            tabIndex={0}
            aria-label={translateUi("横向世界时间线，可左右滚动")}
          >
                  ...
                  return (
                    <li key={`${item.year}-${item.event}-${index}`} className="relative min-w-0">
                      ...
                      <div
                        ...
                        aria-label={translateUi("第 {{value0}} 个事件", { value0: index + 1 })}
                      >
                        {index + 1}
                      </div>
                    </li>
          ...
          <div className="px-5 py-5 md:hidden">
            <ol className="relative ml-4 border-l border-border pl-7" aria-label={translateUi("世界时间线")}>
              {items.map((item, index) => (
          ...
          <div className="flex items-center gap-2 border-t border-border/25 px-5 py-3 text-xs text-muted-foreground">
            ...
            <span>{translateUi("事件按世界设定中的时间顺序排列")}{items.length > 4 ? translateUi("，可左右滚动查看全部阶段") : ""}{translateUi("。")}</span>
          </div>
        </>
      )}
    </FullscreenView>
