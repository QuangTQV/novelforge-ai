...
export default function WorldConsistencyTab(props: WorldConsistencyTabProps) {
  ...
  return (
    <section className="space-y-6">
        <div>
          <h2 className="text-xl font-semibold tracking-tight">{translateUi("检查世界一致性")}</h2>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">{translateUi("确认规则、题材信号、力量体系和冲突来源能共同支撑同一个故事。")}</p>
        </div>

        <div className="flex flex-col gap-4 rounded-3xl bg-primary/[0.055] p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="font-medium">{translateUi("让 AI 阅读整份世界手册")}</div>
            <div className="mt-1 text-xs leading-5 text-muted-foreground">

              {translateUi("发现设定互相冲突或无法支撑剧情时，会给出可逐条处理的问题。")}
            </div>
          </div>
          <Button className="shrink-0 rounded-full" onClick={onCheck} disabled={checkPending}>
            {checkPending ? translateUi("检查中...") : translateUi("运行手册体检")}
          </Button>
        </div>
          ...
          <div className="rounded-3xl border border-border/35 bg-card/70 p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="shrink-0 text-left sm:text-right">
                ...
                <div className="text-xs text-muted-foreground">{translateUi("一致性分数")}</div>
              </div>
            </div>
            <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 border-t border-border/30 pt-4 text-sm text-muted-foreground">
              <span><strong className="font-semibold tabular-nums text-foreground">{openIssues.length}</strong>  {translateUi("项待处理")}</span>
              <span><strong className="font-semibold tabular-nums text-foreground">{errorCount}</strong>  {translateUi("项严重")}</span>
              <span><strong className="font-semibold tabular-nums text-foreground">{warnCount}</strong>  {translateUi("项提醒")}</span>
              <span><strong className="font-semibold tabular-nums text-foreground">{resolvedCount + ignoredCount}</strong>  {translateUi("项已处理")}</span>
              <span className="text-xs">{report.generatedAt ? new Date(report.generatedAt).toLocaleString() : translateUi("检查时间未知")}</span>
            </div>
          </div>
        ) : (
          <div className="flex min-h-44 flex-col items-center justify-center rounded-3xl bg-muted/20 px-6 text-center">
            <div className="font-medium">{translateUi("还没有一致性检查结果")}</div>
            <div className="mt-1 text-sm text-muted-foreground">{translateUi("运行检查后，这里会显示整体判断和需要处理的问题。")}</div>
          </div>
        )}
        ...
        {issues.length > 0 ? (
          <div className="grid gap-4 lg:grid-cols-[270px_minmax(0,1fr)]">
            <div className="space-y-2 rounded-3xl bg-muted/20 p-3">
              <div className="px-2 py-1 text-sm font-medium">{translateUi("问题清单")}</div>
              {issues.map((issue) => {
          ...
          report ? <div className="rounded-2xl bg-success/[0.055] px-4 py-3 text-sm text-muted-foreground">{translateUi("没有需要逐条处理的一致性问题。")}</div> : null
        )}
    </section>
  );
}
