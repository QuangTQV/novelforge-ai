...
export default function WorldDeepeningTab(props: WorldDeepeningTabProps) {
  ...
  return (
    <section className="space-y-6">
        <div>
          <h2 className="text-xl font-semibold tracking-tight">{translateUi("补齐关键设定")}</h2>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">{translateUi("AI 会从手册中找出最影响故事成立的空白，你只需选择方向或用一句话回答。")}</p>
        </div>

        <div className="flex flex-col gap-4 rounded-3xl bg-primary/[0.055] p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="font-medium">{translateUi("寻找下一批关键问题")}</div>
            <div className="mt-1 text-xs leading-5 text-muted-foreground">

              {translateUi("每次只聚焦少量高影响问题，回答会整合进规则、势力、地点或冲突设定。")}
            </div>
          </div>
          <Button className="shrink-0 rounded-full" onClick={onGenerate} disabled={generatePending}>
            {generatePending ? translateUi("生成中...") : translateUi("生成补齐问题")}
          </Button>
        </div>
        ...
        {questions.length > 0 ? (
          <div className="grid gap-4 lg:grid-cols-[250px_minmax(0,1fr)]">
            <div className="space-y-2 rounded-3xl bg-muted/20 p-3">
              <div className="flex items-center justify-between gap-2">
                <div className="px-2 text-sm font-medium">{translateUi("待补问题")}</div>
              ...
              {questions.map((question, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-medium text-foreground">{translateUi("问题")} {index + 1}</span>
                      <span className={answered ? "text-xs text-primary" : "text-xs text-muted-foreground"}>
                        {answered ? translateUi("有回答") : translateUi("待回答")}
                      </span>
                    </div>
              ...
              <div className="space-y-4 rounded-3xl border border-border/35 bg-card/70 p-5">
                <div>
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    ...
                    {translateUi("这条回答会用于补齐世界手册。")}
                  </div>
                </div>
                {activeQuickOptions.length > 0 ? (
                  <div className="space-y-2">
                    <div className="text-xs text-muted-foreground">{translateUi("可直接采用的回答方向")}</div>
                  ...
                  <div className="rounded-2xl bg-muted/20 p-3 text-xs text-muted-foreground">
                    ...
                    {translateUi("可以直接写你的设定答案，也可以先用一句话描述方向。")}
                  </div>
                )}
                <textarea
                  ...
                  placeholder={translateUi("填写这条设定补充")}
                />
              </div>
            ) : null}
          </div>
          ...
          <div className="flex min-h-48 flex-col items-center justify-center rounded-3xl bg-muted/20 px-6 text-center">
            <div className="font-medium">{translateUi("等待发现世界中的关键空白")}</div>
            <div className="mt-1 text-sm text-muted-foreground">{translateUi("生成问题后，可以逐条选择建议方向或补充自己的设定。")}</div>
          </div>
        )}
        <div className="flex justify-end">
          ...
          >
            {submitPending ? translateUi("整合中...") : translateUi("提交并整合回答")}
          </Button>
        </div>
    </section>
  );
