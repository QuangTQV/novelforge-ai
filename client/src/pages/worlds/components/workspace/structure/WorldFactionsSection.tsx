  ...
  return (
    <div className="rounded-md border p-3 space-y-3">
      <div className="flex items-center justify-between">
        <div className="font-medium">{translateUi("阵营与势力")}</div>
      ...
      <div className="rounded-md border border-dashed p-3 text-xs text-muted-foreground space-y-1">
        <div>{translateUi("阵营 = 抽象立场、路线或世界站队；势力 = 具体组织、圈层、网络或机构。")}</div>
        <div>{translateUi("像“社会压力机制”“行业运作规则”“人际网络法则”这类世界级默认规则，应优先写到“规则中心”，不要塞进阵营卡。")}</div>
        <div>

          {translateUi("当前阵营 ID：")}{
            draftStructure.factions.length > 0
              ? draftStructure.factions.map((item) => translateUi("{{value0}}（{{value1}}）", { value0: item.id, value1: item.name || translateUi("未命名") })).join(translateUi("、"))
              : translateUi("暂无")
          }
        </div>
        <div>
          ...
          {translateUi("当前势力 ID：")}{
            draftStructure.forces.length > 0
              ? draftStructure.forces.map((item) => translateUi("{{value0}}（{{value1}}）", { value0: item.id, value1: item.name || translateUi("未命名") })).join(translateUi("、"))
              : translateUi("暂无")
          }
        </div>
      </div>
        ...
        {draftStructure.factions.map((faction, index) => (
          <div key={faction.id || index} className="rounded-md border p-3 space-y-2">
            <div className="text-xs text-muted-foreground">
              ...
              {translateUi("阵营卡描述的是抽象站队，不是具体公司、部门或人脉网络。")}
            </div>
            <Input
              value={faction.name}
              onChange={(event) =>
              ...
              placeholder={translateUi("阵营名称，例如：体制内求稳派 / 市场逐利派 / 关系网络实用派")}
            />
            <Input
              ...
              placeholder={translateUi("立场 / 世界站队")}
            />
            <textarea
              ...
              placeholder={translateUi("阵营理念 / 信条 / 主张")}
            />
            <div className="grid gap-2 md:grid-cols-2">
              <Input
                value={faction.goals.join(translateUi("、"))}
                onChange={(event) =>
                  setDraftStructure((prev) =>
                    prev
                      ? {
                ...
                placeholder={translateUi("长期目标，使用顿号或逗号分隔")}
              />
              <Input
                value={faction.methods.join(translateUi("、"))}
                onChange={(event) =>
                  setDraftStructure((prev) =>
                    prev
                      ? {
                ...
                placeholder={translateUi("常用手段，使用顿号或逗号分隔")}
              />
            </div>
            <Input
              value={faction.representativeForceIds.join(translateUi("、"))}
              onChange={(event) =>
                setDraftStructure((prev) =>
                  prev
                    ? {
              ...
              placeholder={translateUi("代表势力 ID，使用顿号或逗号分隔")}
            />
            {faction.representativeForceIds.length > 0 ? (
              <div className="text-xs text-muted-foreground">

                {translateUi("代表势力：")}{faction.representativeForceIds.map((id) => forceNameById.get(id) || id).join(translateUi("、"))}
              </div>
            ) : null}
          </div>
        ))}
        {draftStructure.forces.map((force, index) => (
          <div key={force.id || index} className="rounded-md border p-3 space-y-2">
            <div className="text-xs text-muted-foreground">
              ...
              {translateUi("势力卡描述的是能施压、能占据地点、能参与关系网络的具体组织或圈层。")}
            </div>
            <div className="grid gap-2 md:grid-cols-3">
              <Input
                ...
                placeholder={translateUi("势力名称，例如：广告公司管理层 / 房屋中介链 / 地方商业圈人脉网")}
              />
              <Input
                value={force.type}
                onChange={(event) =>
                ...
                placeholder={translateUi("势力类型，例如：公司 / 部门 / 中介网络 / 商业圈层")}
              />
              <Input
                ...
                placeholder={translateUi("所属阵营 ID（可空）")}
              />
            </div>
            {force.factionId ? (
              <div className="text-xs text-muted-foreground">

                {translateUi("所属阵营：")}{factionNameById.get(force.factionId) || force.factionId}
              </div>
            ) : null}
            <textarea
              className="min-h-[80px] w-full rounded-md border bg-background p-2 text-sm"
              ...
              placeholder={translateUi("势力概述 / 对外身份 / 在世界中的作用")}
            />
            <div className="grid gap-2 md:grid-cols-2">
              <Input
                ...
                placeholder={translateUi("权力基础 / 资源来源 / 控制抓手")}
              />
              <Input
                ...
                placeholder={translateUi("当前目标 / 眼下想推进什么")}
              />
            </div>
            <div className="grid gap-2 md:grid-cols-2">
              <Input
                ...
                placeholder={translateUi("领导者 / 关键人物（可空）")}
              />
              <Input
                ...
                placeholder={translateUi("施压方式 / 高压来源 / 它如何逼迫角色")}
              />
            </div>
            <div className="grid gap-2 md:grid-cols-1">
              <Input
                ...
                placeholder={translateUi("叙事角色，例如：压迫源 / 诱导者 / 守门人 / 缓冲带")}
              />
            </div>
          </div>
        ))}
