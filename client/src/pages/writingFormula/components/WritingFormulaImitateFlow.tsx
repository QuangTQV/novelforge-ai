...
export default function WritingFormulaImitateFlow(props: WritingFormulaImitateFlowProps) {
  ...
  return (
    <Card className="border-slate-200/80 bg-white/90 shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
      <CardHeader>
        <CardTitle>{translateUi("模仿一种写法")}</CardTitle>
        <div className="text-sm leading-7 text-muted-foreground">

          {translateUi("你先给我一段明确的参考文本，我把它拆成可执行写法，再决定是贴近模仿、保留骨架，还是只迁移可复用部分。")}
        </div>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(300px,0.9fr)]">
          <section className="space-y-3 rounded-2xl border bg-slate-50/70 p-4">
            <div className="text-sm font-medium text-slate-900">{translateUi("1. 输入参考文本")}</div>
            <div className="grid gap-3 md:grid-cols-2">
              <input
                className="rounded-md border bg-white p-2 text-sm"
                placeholder={translateUi("写法名称，例如：冷感现实对话流")}
                value={form.name}
            ...
            <textarea
              ...
              placeholder={translateUi("粘贴你想模仿的参考文本。建议至少给一段完整场景，让系统能看出叙事距离、对白手感和句式节奏。")}
            ...
            <div className="flex justify-end">
              <Button type="button" onClick={onExtract} disabled={!form.name.trim() || !form.sourceText.trim() || extractPending}>
                {extractPending ? translateUi("正在提取特征...") : translateUi("2. 先提取特征")}
              </Button>
            </div>
          </section>

          <section className="space-y-3 rounded-2xl border bg-white p-4">
            <div className="text-sm font-medium text-slate-900">{translateUi("2. 选择保留程度")}</div>
            {draft ? (
              <>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          ...
                          {active ? <Badge variant="secondary" className="bg-white/10 text-white">{translateUi("当前方案")}</Badge> : null}
                        </div>
                ...
                <div className="rounded-xl border bg-amber-50/80 p-3 text-xs leading-6 text-amber-900">
                  ...
                  {translateUi("`imitate` 更适合临摹试写，`balanced` 适合大多数项目，`transfer` 更适合整书绑定，能主动避开高指纹风险。")}
                </div>
              </>
            ) : (
              <div className="rounded-xl border border-dashed p-4 text-sm leading-7 text-muted-foreground">

                {translateUi("先完成特征提取，这里才会出现三种保留策略和对应的读感说明。")}
              </div>
            )}
          </section>
        </div>
        ...
        <section className="rounded-2xl border bg-white p-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="text-sm font-medium text-slate-900">{translateUi("3. 预览即将保存的写法骨架")}</div>
              <div className="mt-1 text-xs leading-6 text-muted-foreground">

                {translateUi("保存后会自动进入当前写法编辑，你可以继续调整规则、绑定到目标，或者直接拿去试写。")}
              </div>
            </div>
            <Button type="button" onClick={onCreate} disabled={!draft || createPending}>
              {createPending ? translateUi("正在保存写法...") : translateUi("4. 保存为写法资产")}
            </Button>
          </div>
          {draft ? (
            ...
            <div className="mt-4 rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
              ...
              {translateUi("还没有可预览的特征。先完成特征提取，再决定保存方式。")}
            </div>
          )}
        </section>
      </CardContent>
