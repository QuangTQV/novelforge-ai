...
export default function WritingFormulaSidebar(props: WritingFormulaSidebarProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4 overflow-y-auto xl:pr-1">
      <Card>
        <CardHeader>
          <CardTitle>{translateUi("先选一套写法再微调")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="text-sm leading-6 text-muted-foreground">

            {translateUi("第一次使用时，不用先理解所有规则字段。先从预置写法里挑一套最像你想写的感觉，再进去改名字、标签和规则，会顺很多。")}
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border bg-muted/20 p-3">
              <div className="text-xs font-medium text-muted-foreground">{translateUi("可直接编辑的写法资产")}</div>
              <div className="mt-1 text-2xl font-semibold text-foreground">{profiles.length}</div>
              <div className="mt-1 text-xs text-muted-foreground">

                {translateUi("其中预置")} {starterProfiles.length}  {translateUi("套，适合直接复制思路后再改。")}
              </div>
            </div>
            <div className="rounded-lg border bg-muted/20 p-3">
              <div className="text-xs font-medium text-muted-foreground">{translateUi("内置模板")}</div>
              <div className="mt-1 text-2xl font-semibold text-foreground">{templates.length}</div>
              <div className="mt-1 text-xs text-muted-foreground">

                {translateUi("适合快速新建一套新写法，不必从空白开始。")}
              </div>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{translateUi("切换当前写法")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="text-xs leading-6 text-muted-foreground">

            {translateUi("这里负责切换弹窗中的编辑对象。完整资产列表在首页查看。")}
          </div>

          {customProfiles.length > 0 ? (
            <div className="space-y-2">
              <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{translateUi("你创建的写法")}</div>
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <div className="truncate font-medium text-foreground">{translateUi(profile.name)}</div>
                    </div>
          ...
          {starterProfiles.length > 0 ? (
            <div className="space-y-2">
              <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{translateUi("预置起步写法")}</div>
              {starterProfiles.map((profile) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Badge variant="outline" className="shrink-0">{translateUi("预置")}</Badge>
                  </div>
                </button>
              ))}
            </div>
          ...
          {profiles.length === 0 ? (
            <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
              ...
              {translateUi("当前还没有写法资产。点上方“新建或导入写法”，先从模板快速起一套最省心。")}
            </div>
          ) : null}
        </CardContent>
      </Card>
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="max-w-5xl">
          <DialogHeader>
            <DialogTitle>{translateUi("新建或导入写法")}</DialogTitle>
            <DialogDescription>

              {translateUi("推荐先走“快速开始”或“空白 / AI”里的句子生成。手里有稳定样本文本时，再使用“从文本提取”。")}
            </DialogDescription>
          </DialogHeader>

          <Tabs value={activeCreateTab} onValueChange={setActiveCreateTab} className="space-y-4">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="quick_start">{translateUi("快速开始")}</TabsTrigger>
              <TabsTrigger value="blank">{translateUi("空白 / AI")}</TabsTrigger>
              <TabsTrigger value="extract">{translateUi("从文本提取")}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="quick_start" className="space-y-4">
              <div className="rounded-lg border bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
                ...
                {translateUi("左侧放了几套可直接修改的起步写法。想再新开一套时，从模板快速生成会更省力，再按项目微调。")}
              </div>
                ...
                {templates.map((template) => (
                  <div key={template.id} className="rounded-lg border p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge variant="outline">{translateUi("模板")}</Badge>
                    </div>
                    ...
                    {template.applicableGenres.length > 0 ? (
                      <div className="mt-3 text-xs text-muted-foreground">
                        ...
                        {translateUi("适合：")}{template.applicableGenres.join(" / ")}
                      </div>
                    ) : null}
                    <Button
                      size="sm"
                    ...
                    >
                      {createFromTemplatePending ? translateUi("创建中...") : translateUi("基于这套快速新建")}
                    </Button>
                  </div>
                ))}
              </div>
            ...
            <TabsContent value="blank" className="space-y-4">
              <div className="rounded-lg border bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
                ...
                {translateUi("这里有两种轻量起步方式：如果你清楚自己要维护一套规则，就手动建空白；如果你只知道“想写成什么感觉”，直接写一句话交给 AI 搭骨架。")}
              </div>
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-lg border p-4">
                  <div className="mb-3">
                    <div className="text-sm font-medium text-foreground">{translateUi("手动空白创建")}</div>
                    <div className="mt-1 text-xs leading-5 text-muted-foreground">

                      {translateUi("适合你清楚自己要维护哪类风格规则，只想先建一个空壳再慢慢补。")}
                    </div>
                  </div>
                  <div className="space-y-3">
                    <input
                      className="w-full rounded-md border p-2 text-sm"
                      placeholder={translateUi("例如：我的女频都市关系写法")}
                    ...
                    >
                      {createManualPending ? translateUi("创建中...") : translateUi("创建空白写法")}
                    </Button>
                  </div>
                </div>
                ...
                <div className="rounded-lg border p-4">
                  <div className="mb-3">
                    <div className="text-sm font-medium text-foreground">{translateUi("AI 帮我先搭一套")}</div>
                    <div className="mt-1 text-xs leading-5 text-muted-foreground">

                      {translateUi("不想先研究规则字段时，直接描述你想要的读感、气质或参考方向，AI 会先生成一套可编辑写法。")}
                    </div>
                  </div>
                  <div className="space-y-3">
                    <input
                      className="w-full rounded-md border p-2 text-sm"
                      placeholder={translateUi("写法名称（可选，不填就让 AI 来取）")}
                      value={createForm.briefName}
                      onChange={(event) => onCreateFormChange({ briefName: event.target.value })}
                    />
                    <input
                      className="w-full rounded-md border p-2 text-sm"
                      placeholder={translateUi("分类（可选）")}
                    ...
                    <textarea
                      ...
                      placeholder={translateUi("例如：类似于《遥远的救世主》的写法，整体克制、思辨感强，对话带锋芒，少鸡汤，多现实摩擦。")}
                    ...
                    >
                      {createFromBriefPending ? translateUi("AI 生成中...") : translateUi("AI 生成一套写法")}
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <TabsContent value="extract" className="space-y-4">
              <div className="rounded-lg border bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
                ...
                {translateUi("适合你手里有一段稳定的参考文本，想让系统先提取特征再进入编辑。没有现成样本时，建议先从模板或 AI 起步。")}
              </div>
              <div className="rounded-lg border p-4">
                <div className="space-y-3">
                  <input
                    className="w-full rounded-md border p-2 text-sm"
                    placeholder={translateUi("写法名称")}
                  ...
                  <textarea
                    ...
                    placeholder={translateUi("粘贴参考文本")}
                  ...
                  >
                    {extractFromTextPending ? translateUi("提取中...") : translateUi("AI 提取特征并创建")}
                  </Button>
                </div>
              </div>
            </TabsContent>
