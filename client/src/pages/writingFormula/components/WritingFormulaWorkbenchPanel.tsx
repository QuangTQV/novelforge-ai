...
export default function WritingFormulaWorkbenchPanel(props: WritingFormulaWorkbenchPanelProps) {
  ...
  const bindingTargetLabel: Record<StyleBinding["targetType"], string> = {
    novel: translateUi("整本书"),
    chapter: translateUi("章节"),
    task: translateUi("本次任务"),
  };
    ...
    <Card className="border-slate-200/80 bg-white shadow-none">
      <CardHeader className="border-b border-slate-100 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle>{translateUi("把写法放进故事里验证")}</CardTitle>
            <div className="mt-1 text-sm text-slate-500">{translateUi("先试读感，再决定让它在哪个创作环节生效。")}</div>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-6 pt-6">
        <div className="flex gap-3 rounded-2xl border border-sky-100 bg-sky-50/70 px-4 py-3 text-sm leading-7 text-slate-700">
          ...
          <span>{translateUi("这里负责绑定与试写。想修正已有正文时，请从“去 AI 味”进入，避免把写法设定和正文处理混在一起。")}</span>
        </div>
        ...
        <div className="space-y-5 rounded-3xl border border-slate-200 bg-[linear-gradient(135deg,rgba(248,250,252,0.96),rgba(255,255,255,0.96))] p-4 md:p-5">
          <div className="flex gap-3">
            ...
            <div className="space-y-1">
              <div className="text-base font-semibold text-slate-950">{translateUi("绑定到目标")}</div>
            <div className="text-sm leading-6 text-slate-500">

              {translateUi("绑定后，这套写法会在对应小说、章节或任务里参与生成。优先级越高，影响越靠前；权重越高，参与程度越强。")}
            </div>
            </div>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <label className="space-y-2">
              <div className="text-sm font-medium text-slate-900">{translateUi("绑定层级")}</div>
              <SelectControl
              ...
              >
                <option value="novel">{translateUi("整本书")}</option>
                <option value="chapter">{translateUi("章节")}</option>
                <option value="task">{translateUi("本次任务")}</option>
              </SelectControl>
            </label>

            <label className="space-y-2">
              <div className="text-sm font-medium text-slate-900">{translateUi("所属小说")}</div>
              <SelectControl
                className="w-full rounded-md border p-2 text-sm"
            ...
            {bindingForm.targetType === "chapter" ? (
              <label className="space-y-2">
                <div className="text-sm font-medium text-slate-900">{translateUi("选择章节")}</div>
                <SelectControl
                  className="w-full rounded-md border p-2 text-sm"
                  value={bindingForm.chapterId}
                  onChange={(event) => onBindingFormChange({ chapterId: event.target.value })}
                >
                  <option value="">{translateUi("选择章节")}</option>
            ...
            {bindingForm.targetType === "task" ? (
              <label className="space-y-2">
                <div className="text-sm font-medium text-slate-900">{translateUi("任务标识")}</div>
                <input
                  className="w-full rounded-md border p-2 text-sm"
                  placeholder={translateUi("例如：chapter-draft-001")}
            ...
            <label className="space-y-2">
              <div className="text-sm font-medium text-slate-900">{translateUi("优先级")}</div>
              <input
                className="w-full rounded-md border p-2 text-sm"
                type="number"
                min={0}
            ...
            <label className="space-y-2">
              <div className="text-sm font-medium text-slate-900">{translateUi("权重")}</div>
              <input
                className="w-full rounded-md border p-2 text-sm"
                type="number"
                min={0.3}
              ...
              bindings.map((binding) => (
                <div key={binding.id} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-3 text-sm">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 truncate text-xs text-slate-500">{translateUi("目标")} {binding.targetId}  {translateUi("· 优先级")} {binding.priority}  {translateUi("· 影响")} {binding.weight}</div>
                  </div>
                  <Button size="sm" variant="ghost" onClick={() => onDeleteBinding(binding.id)}>{translateUi("删除")}</Button>
                </div>
              ))
            ) : (
              <div className="rounded-xl border border-dashed px-3 py-3 text-sm leading-6 text-slate-500">

                {translateUi("这套写法还没有绑定到任何目标。先绑定到小说或章节，后面的生成链路才会自动带上它。")}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-5 rounded-3xl border border-slate-200 p-4 md:p-5">
          <div className="flex gap-3">
            ...
            <div className="space-y-1">
              <div className="text-base font-semibold text-slate-950">{translateUi("先试写一段")}</div>
            <div className="text-sm leading-6 text-slate-500">

              {translateUi("不确定这套写法到底有没有落地成功时，先生成一段或改写一段，是最直观的验证方式。")}
            </div>
            </div>
          </div>

          <label className="space-y-2">
            <div className="text-sm font-medium text-slate-900">{translateUi("试写方式")}</div>
            <SelectControl
              className="w-full rounded-md border p-2 text-sm"
              value={testWriteForm.mode}
              onChange={(event) => onTestWriteFormChange({ mode: event.target.value as "generate" | "rewrite" })}
            >
              <option value="generate">{translateUi("生成正文")}</option>
              <option value="rewrite">{translateUi("改写文本")}</option>
            </SelectControl>
          </label>

          {testWriteForm.mode === "generate" ? (
            <label className="space-y-2">
              <div className="text-sm font-medium text-slate-900">{translateUi("试写主题")}</div>
              <input
                className="w-full rounded-md border p-2 text-sm"
                placeholder={translateUi("例如：主角第一次公开翻盘")}
            ...
            <label className="space-y-2">
              <div className="text-sm font-medium text-slate-900">{translateUi("待改写文本")}</div>
              <textarea
                className="min-h-[140px] w-full rounded-md border p-2 text-sm"
                placeholder={translateUi("粘贴你想用这套写法改写的正文")}
          ...
          <Button onClick={onRunTestWrite} disabled={testWritePending || !selectedProfileId}>
            ...
            {testWritePending ? translateUi("正在试写...") : translateUi("开始试写")}
          </Button>
          ...
          {testWriteOutput ? (
            ...
            <div className="rounded-xl border border-dashed px-3 py-3 text-sm leading-6 text-slate-500">
              ...
              {translateUi("这里会显示试写结果。你可以用它判断这套写法的推进感、对白质感和整体语气是否已经到位。")}
            </div>
          )}
        </div>
      </CardContent>
