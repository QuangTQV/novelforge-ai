  ...
  render: (input, context) => {
    ...
    return [
      new SystemMessage([
        ...
        pick(lang,
          "3a. 如上下文给出「流派模式约束」，本章的状态变化必须取自其「每章可选状态变化清单」；反转/揭示的强度与频率必须匹配其「反转/揭示密度」——dense 表示读者应频繁获得真相分层或认知反转，rare 表示主要靠关系与情绪推进、不要写惊吓式反转；章末钩子按其「章末钩子类型」处理。",
          "3a. Nếu ngữ cảnh có \"Ràng buộc Story Mode\", thay đổi trạng thái của chương phải lấy từ \"Danh sách state change hợp lệ mỗi chương\" của nó; cường độ và tần suất đảo chiều/hé lộ phải khớp \"Mật độ đảo chiều/hé lộ\" — dense nghĩa là người đọc nên thường xuyên nhận sự thật phân tầng hoặc đảo nhận thức, rare nghĩa là chủ yếu đẩy bằng quan hệ và cảm xúc, không viết cú lật gây sốc; móc câu cuối chương xử lý theo \"Kiểu móc câu cuối chương\" của nó.",
          "3a. If the context provides \"Story-mode constraints\", this chapter's state change must be drawn from its \"Valid per-chapter state-change menu\"; the intensity and frequency of reversals/reveals must match its \"Reversal / reveal density\" — dense means the reader should frequently get layered truth or perception reversals, rare means advance mainly through relationship and emotion with no shock twists; the chapter-ending hook follows its \"Chapter-ending hook style\"."),
        "4. " + endingHook,
        "",
