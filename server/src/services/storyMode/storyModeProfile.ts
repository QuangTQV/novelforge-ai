...
const STORY_MODE_BLOCK_LABELS: Record<PromptLanguage, StoryModeBlockLabels> = {
  ...
  vi: {
    ...
    twistIntensity: "Sức nặng cú lật",
    twistFairness: "Độ công bằng của cú lật",
    twistScope: "Tầng bị cú lật tác động",
    ...
    engineNote: "Yêu cầu động cơ tự sự: lần lập kế hoạch và viết này phải tuân thủ 9 field động cơ tự sự bên dưới (nguồn lực đẩy truyện / danh sách state change / mật độ, sức nặng, độ công bằng, tầng tác động và cơ chế của cú lật / thời gian giữ foreshadow / kiểu móc câu cuối chương), ưu tiên hơn thói quen kể chuyện chung chung; quy tắc thực thi cụ thể do bước tách chương và viết chương đưa ra.",
    ...
    scope: {
      ...
      worldview_shattering: "worldview_shattering (lật đổ nhận thức về luật lệ thế giới hoặc cục diện tổng thể)",
    },
    mechanism: {
