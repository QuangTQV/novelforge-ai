...
const DEFAULT_STORY_MODE_PROFILE: StoryModeProfile = {
  coreDrive: "Đẩy trải nghiệm đọc dài kỳ bằng việc trả về ổn định kỳ vọng đọc cốt lõi.",
  ...
  chapterUnit: "Mỗi chương xoay quanh một đơn vị đẩy truyện rõ ràng.",
...
const STORY_MODE_BLOCK_LABELS: Record<PromptLanguage, StoryModeBlockLabels> = {
  zh: {
    ...
    engineNote: "叙事引擎要求：本次规划与写作必须遵守下方「章节推进动力来源 / 每章可选的状态变化清单 / 反转/揭示密度 / 伏笔保留时长 / 章末钩子类型」五项，其优先级高于泛化叙事习惯；具体执行规则由拆章与写章步骤给出。",
  ...
  vi: {
    ...
    engineNote: "Yêu cầu động cơ tự sự: lần lập kế hoạch và viết này phải tuân thủ 5 mục bên dưới (\"Nguồn lực đẩy truyện mỗi chương\" / \"Danh sách state change hợp lệ mỗi chương\" / \"Mật độ đảo chiều / hé lộ\" / \"Thời gian giữ foreshadow\" / \"Kiểu móc câu cuối chương\"), ưu tiên hơn thói quen kể chuyện chung chung; quy tắc thực thi cụ thể do bước tách chương và viết chương đưa ra.",
  ...
  en: {
    ...
    engineNote: "Narrative-engine requirement: this planning and writing pass must obey the five fields below (\"Per-chapter momentum source\" / \"Valid per-chapter state-change menu\" / \"Reversal / reveal density\" / \"Foreshadow hold duration\" / \"Chapter-ending hook style\"); they take priority over generic narrative habits. The concrete execution rules are given by the chapter-split and chapter-writing steps.",
